...
- **ステータス:** **Superseded by [ADR-014](ADR-014-syntax-engine-tree-sitter.md)** (2026-07-21、Phase 7a 着手前レビュー)
- **決定日:** 2026-07-14

> ⚠️ **本 ADR は [ADR-014](ADR-014-syntax-engine-tree-sitter.md) によって置き換えられました。**
> Phase 7a 着手前調査で、本 ADR の「`.tmLanguage.json` はコピペで導入可能」という根拠が文法**定義ファイル**の再利用可能性の話であり、それを解釈する**インタプリタ**の C++ 向け実装が存在しないことを見落としていたと判明した (TypeScript/C#/Java にしか成熟した実装が無い)。詳細は ADR-014 §コンテキスト を参照。
