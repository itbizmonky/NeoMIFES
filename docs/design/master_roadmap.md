...
| 5c3〜 | Grep結果ペインUI / タグジャンプ / 検索履歴 | ⏭️ 次候補 | §5.5 |
...
#### 実装後の確定事項/変更点 (2026-07-20、Phase 5c1・5c2完了)

**§5.5全体はGrep・複数フォルダ検索・検索履歴・タグジャンプ・秀丸互換Grep結果ペインを1章にまとめているが、実装は4b8・5b3と同じ要領でサブフェーズへ分割した。** 本節は`search::GrepService`コア(ヘッドレス、UIなし、Phase 5c1)と、実行時ファイルを開く機能(`neomifes::app::openDocumentAt()`、ヘッドレス、Phase 5c2)が完了した状態を記す。他のサブフェーズ(結果ペインUI・タグジャンプ・検索履歴)は引き続き未着手。
