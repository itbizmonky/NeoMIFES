...
| 4b8 | 矩形・タブ変換・分配・フリーカーソル・マーカー | ⏸️ 保留 | §3 |
| 5a | Search Engine 基盤 | ✅ 完了 | — |
| 5b1 | 複数行マッチ対応 | ✅ 完了 | — |
| 5b2 | 置換 (ReplaceAllCommand + キャプチャ) | ⏭️ 次候補 | §4 |
| 5b3 | Find bar UI + コマンドパレット | 未着手 | §5 |
| 5c | Grep / 複数フォルダ / タグジャンプ | 未着手 | §5.5 |
| 6 | エンコーディング + 自動判定 + 10GB mmap | 未着手 | §6 |
...
### 5.8 実装後の確定事項/変更点 (2026-07-19、Phase 5b3a・5b3b 完了)

**§5全体はFind bar + コマンドパレット + Grepをまとめて記述していたが、実装は3段階(5b3a/5b3b/5b3c)に分割した。** 本節は§5.1-5.4(Find bar UI基盤)に対応する**Phase 5b3a**と、置換行配線(Ctrl+H)に対応する**Phase 5b3b**が完了した状態を記す。§5.5(Phase 5c、Grep)・コマンドパレット(§5.2後半、Phase 5b3c)は未着手のまま。
...
- **意図的にスコープ外とした項目 (Phase 5b3c/5cへ延期):** コマンドパレット(Ctrl+Shift+P)、Case/Word/Regexのクリック可能なトグルボタン(Alt+C/W/Rキーバインドのみ実装)、クリックできるReplace/Allボタン(キーバインドのみ)、検索履歴、タグジャンプ、Grep。UIの消費者が無い状態でこれらを作るのはCLAUDE.mdルール3の推測実装にあたるため
