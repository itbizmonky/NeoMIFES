...
| 6b2〜 | ISO-2022-JP・Document統合・10GB mmap | ⏭️ 次候補 | §6 |
...
#### 実装後の確定事項/変更点 (2026-07-20、Phase 5c1・5c2・5c3・5c4完了)

**§5.5全体はGrep・複数フォルダ検索・検索履歴・タグジャンプ・秀丸互換Grep結果ペインを1章にまとめているが、実装は4b8・5b3と同じ要領でサブフェーズへ分割した。** 本節は`search::GrepService`コア(ヘッドレス、UIなし、Phase 5c1)、実行時ファイルを開く機能(`neomifes::app::openDocumentAt()`、ヘッドレス、Phase 5c2)、Grep結果ペインUI(`ui::GrepBar`、Ctrl+Shift+F、Phase 5c3)、タグジャンプ(`util::parseTagJumpReference`、F12、Phase 5c4)が完了した状態を記す。他のサブフェーズ(検索履歴永続化)は引き続き未着手。
...
**Phase 6全体の残りはPhase 6b2(ISO-2022-JP、正確性トレードオフへの対応方針が未決定)とPhase 6d(Document/OriginalBuffer統合、10GB mmap一般化)のみ。Phase 5c5(検索履歴永続化)はこのセッション冒頭で「Phase 6優先」の承認を得ており、Phase 6完了までは次フェーズ候補として並記しない方針を明確化した。**
