...
### 3.7 実装後の確定事項/変更点 (2026-07-19、Phase 4b8a 完了)

**§3全体は矩形選択・フリーカーソル・マーカー・桁位置ジャンプ・タブ⇔スペース変換・N対N分配クリップボードの6機能を1章にまとめていたが、実装は5b3同様サブフェーズへ分割した。** 本節は**矩形選択の基本機能のみ**(Phase 4b8a)が完了した状態を記す。他の5機能は引き続き未着手。
...
- **フリーカーソル(仮想空間・虚数位置)は対象外、短い行は単純に実際の行長でクランプ。** roadmapモックアップの「短い行は右側の薄い矩形が仮想空間」という視覚表現も未実装 — フリーカーソル機能自体が別サブフェーズであるため
...
- **意図的にスコープ外とした項目 (Phase 4b8の後続サブフェーズへ):** キーボードでの矩形選択拡張(`Alt+Shift+矢印` — `MainWindow`が`WM_SYSKEYDOWN`を未処理のため新規フック要)、フリーカーソル、`Shift+Alt+I`変換、マーカー(Bookmark)、桁位置ジャンプ、タブ⇔スペース変換、N対N分配クリップボードの高度な設定
