...
> ⚠️ **v2.1 でフェーズ順序を最後尾へ移動した。** 理由: CLAUDE.md が「AI 機能は完全プラグイン化。エディタ本体は AI 無しでも 100% 動作しなければならない」と定めているが、本体が 100% 動作していない段階で AI を積むのはこの原則と矛盾する。加えて AI 機能は外部 API 依存で陳腐化が速く、本体完成後に実装した方が製品価値が高い。**着手は Phase 8.5 / 8.6 / 12' / 10 / 11 の完了後。**
...
> ✅ **実装後の確定事項 (WI-13着手、2026-08-16):** 上記チェックリストの最新の達成状況は `build_plan.md` §6 (同一内容、WI-13の実施記録として更新される正の情報源) を参照。ここでの重複更新は行わない — CLAUDE.md §11 が警告する「複数箇所の片方だけ更新し陳腐化する」パターンを避けるため。要点: 14項目中9項目達成 (起動時間29.3ms実測・60fps維持・10GB実ファイル開封を含む)、5項目が未達/進行中(8時間ソーク進行中・ASan結果待ち・本物のAuthenticode証明書未取得(自己署名で機構実証済み)・日常的ドッグフーディングは限定的)。🎉 M4の正式達成は8時間ソーク結果確認後にユーザーと共に判断する。
