...
| 複数カーソル | ✕ | △ | ✕ | 4b (完了、既に凌駕) |
| 矩形選択 | ○ | ○ | ◎ | 4b8 |
| 縦編集 (列編集、MIFES由来 - 「縦書き入力」ではない、`docs/issues/master_roadmap_vertical_edit_terminology_drift.md`参照) | ○ | ○ | ◎ | **WI-26で完了(2026-09-04)** — 矩形選択(`SelectionModel::setRectangularSelection()`)への挿入/削除/上書きは既存の汎用マルチカーソル編集機構が新規コード無しで正しく処理することをコード監査+結合テストで確認、行末一括追記のみ新規`column.append`パレットコマンドとして実装(4コマンド構想ではなく1コマンドで充足、詳細は`detailed_design.md` §5.1.1) |
| フリーカーソル (虚数位置) | ✕ | ◎ | ○ | 4b8 (拡張) |
| タブ⇔スペース変換 | ○ | ○ | ◎ | 4b8 (完了) |
...
>
> 🧊🧊 **(2026-09-11 追記、別種の凍結が重なっている点に注意) 上記の「着手条件」(Phase 8.5/8.6/12'/10/11の完了)は、この文書の§2フェーズ表が示す通り全て満たされた。しかし本フェーズは依然として着手していない——2026-09-11、NeoMIFES プロジェクト全体が別理由(AI差別化は専門企業によるコモディティ化で個人開発が勝負できる領域ではないとユーザーが判断)で凍結されたため。** つまり「フェーズ順序上の前提条件」(2026-08-23時点の凍結理由)と「プロジェクト自体の凍結」(2026-09-11)は別の凍結であり、前者の条件充足は後者を解除しない。プロジェクト全体の凍結状況・再開条件は [`docs/phase_reports/project_freeze_2026-09-11.md`](../phase_reports/project_freeze_2026-09-11.md) を参照。
...
- [x]/[ ] 100 万 Undo (24 時間ソーク、メモリ膨張無し) — **100万Undo自体の能力・速度は`BM_UndoStack_PushOneMillion`(412.5ms)/`BM_UndoStack_UndoOneMillion`(267.1ms)で実測済み、達成。** 「24時間ソーク」部分は、ユーザー指摘(2026-08-31)により**実際にUndo/Redoを回す能動的なストレステストではなく、起動放置によるアイドル安定性確認である**ことが判明し、[`undo_redo_active_usage_soak_not_performed.md`](../issues/undo_redo_active_usage_soak_not_performed.md)(P2)として起票した。アイドルソーク自体の結果は下記「クラッシュ0」項目を参照(こちらへ統合)。**🟢 2026-09-01、同issueをヘッドレスプローブ(`core::UndoStack`/`document::Document`直接駆動)による能動的ソークで解決済み。** 5分間・約14億回のpush/undo/redo/undo操作を実測し、`UndoStack`自体が予期しない形でリークしないことを確認(`push()`の`m_redo.clear()`が正しく動作)。観測された緩やかな線形増加(非加速、1pushあたり約4.06バイト)は`AddBuffer`の既知のappend-only設計に起因する別事象で、[`undo_stack_unbounded_memory.md`](../issues/undo_stack_unbounded_memory.md)(P2)で引き続き追跡中。文字通り24時間・実UIを通した能動ソークは(既知の線形トレンドの追認にしかならず新たな知見が無いため)実施していない
- [x] クラッシュ0/メモリ安定性(アイドル12時間ソーク) — **完了・達成(2026-08-31 11:04:38〜23:04:39 JST、正味12時間00分01秒)。** タスクスケジューラ(`NeoMIFES_V1_SoakTest`)で15分おき48回、署名済みReleaseバイナリの生存確認+WorkingSet記録を実施(`.tmp_v1_verify\soak_log.csv`)、全区間`Responding=True`、`SOAK_COMPLETE_NO_CRASH`を記録しタスク自身が自動的に登録解除・プロセス終了。**WorkingSet推移: 50.71MB(起動直後)→57.58MB(定常、約5時間)→95〜97MB(16:19以降、Application Verifier検証での一時再起動を挟んだ後の水準、以降は単調増加なく横ばいで安定)。** 途中`INTENTIONAL_RESTART_FOR_APPVERIFIER_TEST_NOT_A_CRASH`と明記された1回の意図的な再起動あり(AppVerifier検証のため、クラッシュではない)。検証範囲は「アイドル状態でのクラッシュ0/メモリ安定性」のみで、実際の入力・編集は含まない(上記issue参照)
- [x]/[ ] 10GB ファイル対応 (通常編集 + ログ解析モード + CSV モード + JSON/XML Tree モード) — **通常編集/ログ解析モードは`decode_cache_unbounded_growth.md`修正後に実機再検証済み**(通常編集: p50/p95維持、ログ解析モード: 10GB/1.23億行でPrivate 4.54GBに収まり応答性維持を確認)。**CSVモードは一時デコードバッファの問題は解消したが、恒常的なper-cellインデックスのメモリコストが別途残る**([csv_per_cell_index_memory_scaling.md](../issues/csv_per_cell_index_memory_scaling.md)、P1) — **🟡 M5達成後、2026-09-01に部分対応。** `CsvCell`を24→16バイト/セルへ圧縮(662MB/1330万行で実機再測定、WorkingSet約1.97GB)、ユーザー承認のもと10GB規模の根本解消(遅延インデックス化)は対象外と確定したため、10GB規模でのリスク自体は軽減されつつも残存する。**JSON/XMLTreeモードは100MB規模で3分以上のUIハングを確認**([json_tree_ui_population_hang.md](../issues/json_tree_ui_population_hang.md)、P1、原因未調査) — **🟢 M5達成後、2026-09-01に解決済み**(`ui::JsonTreePane`のしきい値ベース遅延ロード化、実測: トグル9ms・145万件展開303ms、詳細は同issue参照)。検証中に別問題(JSON構文ハイライトが大規模ファイルのオープン自体を約47秒ハングさせる、`json_tree_ui_population_hang.md`とは無関係)を新規発見、[`json_syntax_highlight_large_file_open_hang.md`](../issues/json_syntax_highlight_large_file_open_hang.md)(P1、未対応)として起票。10GB規模のJSON/XML Tree検証はこの新issueにより依然未実施
- [x] 数 GB Grep ≤ 30 秒 — **達成。** 当初(2026-08-31)標準プローブで`SearchService::findAll()`(3GB単一ファイル)38.94秒、`GrepService::findAll()`(1.49GB/5000ファイル)23.87秒と目標超過を確認、[`search_grep_multi_gb_performance_gap.md`](../issues/search_grep_multi_gb_performance_gap.md)(P1)として起票。実測で真因はRE2ではなくUTF-8変換処理(単一ファイル側)/`detectLineEndingBounded()`の冗長デコード(多ファイル側)と判明——2026-09-01にUTF-8変換のASCII高速パス追加で3GBが約17〜18秒(loadFile含む、目標30秒以内)、**2026-09-04(WI-23)に`GrepService`側のファイルあたり固定オーバーヘッドも解消**(専用ローダ`loadUtf8FileForGrep()`新設等3件のFix)。恒久ベンチマーク`tests/bench/grep_service_bench.cpp`も追加し、issueの完了条件を全て達成・解決済みへ移動した
...
- [x]/[ ] 基本アクセシビリティ (キーボード完結ナビゲーション・高コントラストモード動作・スクリーンリーダーでの基本疎通確認。専門認証は求めない) — **部分達成、UI Automationによる実機検証済み(2026-08-31)。** メニューバー(`IsKeyboardFocusable=True`、`ExpandCollapsePattern`対応)・ステータスバー・タブ・クラッシュ復旧ダイアログ(`InvokePattern`経由のプログラム的操作が実際に成功)は全て名前付きで正しく公開されている。高コントラストテーマは既存(WI-09で実機確認済み、ただしOSの高コントラスト設定への自動追従は無く手動選択のみ)。**しかし主要なテキスト編集領域(Direct2D直接描画)がUI Automationへ一切内容を公開していない**(`ControlType.Custom`、`Name=''`)ことを新規に発見、[`text_surface_no_screen_reader_exposure.md`](../issues/text_surface_no_screen_reader_exposure.md)(P1)として起票した。スクリーンリーダーユーザーはメニュー操作はできてもファイル内容を読めない状態。**🟢 2026-09-02、同issueを「簡易アナウンス実装」(フルTextPattern実装ではなく)で解決。** `ui::TextSurfaceAccessible`(自前`IAccessible`)+`WM_GETOBJECT`+カーソル行変化時の`NotifyWinEvent(EVENT_OBJECT_LIVEREGIONCHANGED, ...)`で、カーソルが移動するたびに現在行の内容をATへ通知する。列単位のキャレット追跡・範囲選択の読み上げはフルTextPattern実装が必要なため引き続きスコープ外
...
**達成状況 (2026-09-04時点、WI-23での数GB Grep解決を反映して更新):** 17項目中17項目達成(うち3項目「基本アクセシビリティ」「10GBファイル対応」「100万Undo」は部分達成 — 100万Undoは能力・速度に加え、ヘッドレスソークで`UndoStack`自体のリーク無しも確認済み。文字通りの24時間実UIソークのみ未実施。基本アクセシビリティはテキスト編集領域の簡易アナウンス実装まで完了、列単位のキャレット追跡等はスコープ外。10GBファイル対応はCSVモードのper-cellインデックスメモリコストが遅延インデックス化見送りのため恒常的に残存)。「数GB Grep」は2026-09-01部分対応→2026-09-04(WI-23)完全解決。アイドル12時間ソークは2026-08-31 23:04:39(JST)に`SOAK_COMPLETE_NO_CRASH`で完了済み。**検証中に7件の重大な発見があった(6件解決済み、1件部分対応。詳細は各項目参照):**
...
4. `search_grep_multi_gb_performance_gap.md`(P1、🟢 2026-09-01部分対応→2026-09-04〔WI-23〕完全解決) — 検索・Grepが3GBで38.94秒、目標30秒を超過。実際の原因はRE2ではなくUTF-8変換処理(単一ファイル)/`detectLineEndingBounded()`の冗長デコード(多ファイル)と判明、ASCII高速パス+GrepService専用ローダ新設で解消、恒久ベンチマークも追加し完了条件を全て達成。
5. `text_surface_no_screen_reader_exposure.md`(P1、🟢 2026-09-02解決済み) — 主要テキスト編集領域がUI Automationへ内容を一切公開していない(スクリーンリーダーがファイル内容を読めない)。`ui::TextSurfaceAccessible`(自前`IAccessible`)による簡易ライブリージョンアナウンス(カーソル移動時に現在行を通知)で解決、フルTextPattern実装は引き続きスコープ外。
6. `undo_redo_active_usage_soak_not_performed.md`(P2、🟢 2026-09-01解決済み) — 「100万Undo(24時間ソーク)」の実体が、実際にUndo/Redoを回さないアイドル放置確認だった(ユーザー指摘で発覚)。ヘッドレスプローブで`core::UndoStack`を直接駆動し5分間・約14億操作の能動的ソークを実施、`UndoStack`自体はリークしないことを確認(`push()`が`m_redo.clear()`を正しく実行)。観測された線形増加(非加速)は既知の`AddBuffer` append-only設計に起因、`undo_stack_unbounded_memory.md`で引き続き追跡中。
...
| ~~**U#21** (v2.1 新規、2026-09-04訂正)~~ | ~~**縦編集(列編集、MIFES由来)専用コマンド4種の実装要否**~~ — **WI-26で解決済み(2026-09-04)。** コード監査の結果、挿入/削除/上書きは既存の汎用マルチカーソル編集機構が新規コード無しで処理していると判明(結合テストで確認)、行末一括追記のみ新規`column.append`コマンドとして実装。4専用コマンド構想は不要と判断した(`detailed_design.md` §5.1.1、`docs/design/build_plan.md` WI-26セクション参照) | 解決済み | — |
