...
- [ ] 🎉 **ドッグフーディング: NeoMIFES で NeoMIFES のソースを開いて編集し、保存し、そのままコミットできた** ← **ユーザーが実施し、2件の実害あるバグを発見・報告 (下記「ドッグフーディングで発覚したバグ」参照)。両バグとも修正し、ユーザーが再確認して問題解消を確認済み (2026-08-05)。ただし、この確認は2バグの再現手順のみが対象で、「NeoMIFES自身のソースを開いて編集・保存・実際にコミットする」という完全なドッグフーディングループはまだ実施されていないため、本項目自体は引き続き未チェック**
