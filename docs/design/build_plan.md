...
> 🧊 **プロジェクト凍結中 (2026-09-11)。** ユーザーの明示的な凍結解除・再開指示が無い限り、以下の手順で新規WIへ着手しないこと。経緯・理由・再開条件は [`docs/phase_reports/project_freeze_2026-09-11.md`](../phase_reports/project_freeze_2026-09-11.md) を参照。
...
>
> **🎉 WI-24(Ctrl+F検索を秀丸風の独立ダイアログ化)完結(2026-09-04)。** WI-23完結後、次フェーズ候補確認でユーザーから「検索バーをダイアログ形式とする改修は実施されていないのか」との質問があり、WI-18(2026-09-02)で既にCtrl+H(置換)のみダイアログ化済みだった経緯を説明、ユーザーから「秀丸エディタのような検索ダイアログが理想」との明確な要望を受け着手した。新規`ui::FindDialog`を`FindReplaceDialog`の骨格からフォークして新設(置換モードのランタイムトグル追加は「1クラス1責務」の既存方針に反するため却下)、`FindBar`から検索履歴/Ctrl+H割り込みの2機能を移植、`normal_mode_wiring.cpp`約200箇所の機械的リネームを実施(コンパイルエラーで置換漏れを検出)。設計段階でチェックボックス行の幅計算バグを手計算で発見・事前修正(`FindReplaceDialog`が過去に踏んだのと同型のバグ)。`ui::FindBar`は呼び出し元ゼロになったため完全削除。実機ドッグフーディングでCtrl+F/ライブ検索/F3/Ctrl+H遷移/Escape/正規表現チェックボックスを全て確認、途中2回遭遇したプロセス終了は`keybd_event`によるグローバルキー合成の既知の環境不安定性と特定し、より確実な入力手法で問題なく再現できることを確認した。Debug/Release/ubsan全1576/1576件green、clang-tidy(変更・新規13ファイル)新規指摘0件。詳細は本ファイルのWI-24セクション参照。
>
> **🎉 WI-25(自動整形/reindent機能の新設)完結(2026-09-04)。** WI-24完結後、ユーザーから「貴方の判断で次に改修する項目を決めて完成版のゴールを目指して欲しい」との指示があり、一般探索エージェントへ完成度監査を委任した結果、要件定義書§6明記の「自動整形」がmaster_roadmap.mdの60機能マトリクスにすら登場せず計画から完全に見落とされていたと判明、これを新規WI-25として実装した。汎用ブレース深度APIが存在しないこのコードベースで、既存`syntax::parse()`のトークン分類(`Punctuation`/`String`/`Comment`)を再利用し文字列/コメント内のブレースを正しく除外する設計とした。新規`app::supportsReindent()`(12言語の明示的許可リスト)+`app::computeReindentEdits()`(`DepthTracker`/`SkipRangeTracker`/`ScopeTracker`の3ヘルパークラスで認知的複雑度を抑制)を新設、`edit.reindent`としてコマンドパレットへ配線(既存`edit.convertTabsToSpaces`と同じパレット専用の足跡)。実機ドッグフーディングでC++ファイルの再インデント・Undo一括巻き戻し・非対応言語でのサイレントno-opを確認。副次的に「縦編集」の用語がmaster_roadmap.md内で無断再定義されていたドキュメント不整合も発見・同日解決した([`master_roadmap_vertical_edit_terminology_drift.md`](../issues/master_roadmap_vertical_edit_terminology_drift.md))。Debug/Release/ubsan全1589/1589件green、clang-tidy新規指摘0件。詳細は本ファイルのWI-25セクション参照。
>
...
## WI-24 — Ctrl+F 検索を秀丸風の独立ダイアログ化(`ui::FindDialog`新設)

### 目的

WI-23完結後の次フェーズ候補確認で「検索バーをダイアログ形式とする改修は実施されていないのか」との質問があり、経緯を調査・説明した。WI-18(2026-09-02)で既に検索UIのダイアログ化自体は実装済みだったが、対象はCtrl+H(置換、`ui::FindReplaceDialog`)のみで、Ctrl+F(単純検索、`ui::FindBar`)は要件定義書§6が「通常検索」と「インクリメンタル検索」を別項目として挙げていることを根拠にVSCode流の埋め込みバーのまま意図的に残されていた。ユーザーから「ダイアログとしたい、秀丸エディタのような検索ダイアログが理想」との明確な要望があり、Ctrl+Fも`FindReplaceDialog`と同じ独立ダイアログ方式へ揃えることになった。

着手前にAskUserQuestionで2点を確認: ①検索トリガー方式は既存`FindReplaceDialog`と同じライブ検索(150msデバウンス自動検索、推奨)を採用、②Ctrl+Up/Downの検索履歴呼び出し(`core::SearchHistory`)は既存`FindBar`の機能を引き継ぐ(推奨)。Plan agentへ詳細設計を委任し、EnterPlanMode/ExitPlanModeで正式なPlan承認を得て着手した。

### 設計判断

新規`ui::FindDialog`クラスを新設し、`FindReplaceDialog`(WI-18で既に実装済み、独立トップレベルウィンドウ)への「置換モード実行時トグル」追加は却下した。理由: (1) WI-18時点で`FindBar`から置換モード一式を意図的に完全削除した経緯があり、同種のトグルを`FindReplaceDialog`側に復活させるのは方針の逆行になる、(2) `FindReplaceDialog`のジオメトリは`constexpr`で`create()`時に1回だけ計算される固定5行レイアウトであり実行時可変化には新規ロジックが必要、(3) このコードベースは全ウィジェットが「1クラス1責務」を貫いている。

### 実装

**新規`ui::FindDialog`**(`src/ui/include/neomifes/ui/find_dialog.h` + `src/ui/src/find_dialog.cpp`)——`find_replace_dialog.h`/`.cpp`を骨格としてフォーク(独自`WNDCLASSEXW`/`wndProcTrampoline`/`wndProc`、`WS_POPUP | WS_CAPTION | WS_SYSMENU`、所有〔非親子〕、150msデバウンス、edit subclass、IME合成ガード)。置換欄・置換ボタン・すべて置換ボタンを削り、レイアウトは5行→4行(検索欄/3チェックボックス/件数ラベル/次を検索・前を検索の2ボタン)。`FindBar`から3機能を移植: `setQueryText()`(Ctrl+Up/Down用)、`onHistoryOlder`/`onHistoryNewer`(`core::SearchHistory`連携)、`onReplaceRequested`(検索欄フォーカス中のCtrl+H→`findReplaceDialog.show(hwnd)`、このダイアログ自身がフォーカスを持つ間は`MainWindow::onKeyDown`へ届かないため必須)。Case/Word/Regexは`FindReplaceDialog`と同じ可視チェックボックスを採用(`FindBar`旧来のAlt+C/W/Rキーボード専用トグルは廃止)。

**ジオメトリ計算で1件のバグを設計時に発見・防止した。** `FindReplaceDialog`のダイアログ幅は「4ボタン行(最も幅が広い行)」を基準に決まっていたが、`FindDialog`はボタンが2つしかないため、ボタン行がもはや最幅ではなく、3チェックボックス行(`kMarginDips + 3*kCheckWidthDips` = 400)がラベル+検索欄行(380)・2ボタン行(222)のいずれよりも幅広いことが判明。`kCheckRowWidthDips`を新規に幅計算へ追加(`std::max({...})`、3値比較のため`readability-avoid-nested-conditional-operator`を避けて`<algorithm>`の`std::max`初期化子リスト版を使用)しなければ、`FindReplaceDialog`が過去に一度踏んだ「最幅でない行を基準に幅を決めてチェックボックスがはみ出す」バグを再発するところだった——**コードを書く前の手計算で発見・修正**、実機ドッグフーディングで実際に400×166pxで3チェックボックス全て正しく収まることを確認した。

**`normal_mode_wiring.cpp`/`.h`の機械的リネーム(最大の変更範囲、約200箇所)。** `FindBar`はCtrl+F専用ではなく、アプリ全体でマッチ件数表示シンクとして約30箇所以上から参照される共有オブジェクトだったため、`FindBar`→`FindDialog`/`findBar`→`findDialog`への一括置換(PowerShell正規表現、大文字小文字を区別する2パス)を実施し、既存の`MatchCountSink`テンプレート(`jumpToMatch`/`refreshMatches`/`runFindQuery`/`navigateToMatch`等、テンプレート自体は無変更)へダックタイピングでそのまま差し込んだ。一括置換後、以下を手動で個別修正: ①`cfg.onResize`の`findBar.onParentResized(...)`呼び出しを削除(`FindDialog`はドッキングしない)、②`cfg.onCommand`の`findBar.handleCommand(...)`呼び出しを削除(`FindDialog`は自身の`wndProc`内で`WM_COMMAND`を独自ルーティング)、③全ての`findDialog.show()`呼び出しに`hwnd`引数を追加(`FindDialog::show()`は`FindReplaceDialog::show(HWND)`と同じくowner引数必須、旧`FindBar::show()`は引数無しだった)、④一括置換が意図せず書き換えてしまった「歴史的にFindBarを指す」コメント(例: 「previously findBar.showWithReplace()」)を手動で元に戻す。**コンパイルエラーが置換漏れの検出網として機能した**(`CommandDispatchContext`のdesignated initializerは全フィールド必須のため、型不一致は黙って通らずビルドエラーになる)。

**約30ファイルのドキュメントコメント更新。** `FindBar`を設計上の比較対象として参照していた兄弟ウィジェット(`GrepBar`/`CommandPalette`/`GotoLineBar`/`CsvGridPane`/`TabBar`/`JsonPathBar`/`OutlinePane`等)のコメントを精査し、単純な`FindBar`→`FindDialog`置換が適切なもの(現在も成立する設計比較)と、`FindReplaceDialog`へ差し替えるべきもの(WI-18より前の`FindBar`が「検索欄+置換欄の2エディット構成」だった頃の記述——WI-18で削除済みの構成に対する既に陳腐化していたコメント、今回追加で発見・修正)を判別して個別修正した。歴史的に正確な記述(「WI-18b当時、`FindBar`と対比してこのクラスが設計された」等)はそのまま残し、`FindBar`自体が指す実体を`FindDialog`へ機械的に置換するだけでは不正確になる箇所を1つずつ判断した。

**`ui::FindBar`(`find_bar.h`/`.cpp`)を完全削除。** 上記リネームが完了すると呼び出し元がゼロになったため、CLAUDE.mdの「使われていないコードは完全に削除してよい」方針に従い削除した(バックワード互換シムは残さない)。

### 実機ドッグフーディング

PowerShell + Win32 P/Invoke(`EnumWindows`/`EnumChildWindows`/`PostMessage`/`SendMessage`/`PrintWindow`)で以下を確認した: Ctrl+F(`WM_COMMAND`直接送信)でダイアログが中央配置で開く(400×166px、3チェックボックス・2ボタン全て正しく収まりクリッピング無し)、`WM_CHAR`によるライブ入力で150msデバウンス後に検索が実行され「1/2」のマッチ件数表示・文書内ハイライトの両方が正しく更新される、F3で次のマッチへ正しく移動(ステータスバー「2/2」・ハイライト位置とも確認)、検索欄フォーカス中のCtrl+Hで`FindReplaceDialog`(「検索と置換」ダイアログ)へ正しく遷移、Escapeでダイアログが閉じる、正規表現チェックボックスON+`ERR\w+`パターンで正しく2件マッチ。

**実機ドッグフーディング中、2回原因不明のプロセス終了に遭遇したが、いずれもコードの不具合ではなく既知の環境制約(`reference_no_win32_gui_automation.md`)と特定した。** `keybd_event`によるOSレベルのCtrl+H同時押し合成の直後に2回、Windowsイベントログにクラッシュ記録が一切残らないままプロセスが終了する事象が発生した。同じCtrl+Hの動作を「`keybd_event`でCtrlを押下状態にしつつ、`PostMessage`で対象コントロールへ直接`WM_KEYDOWN('H')`を届ける」という、フォーカス依存のグローバルキー合成を避けたより確実な方式に切り替えたところ、プロセスは終了せず`FindReplaceDialog`が正しく開くことを複数回にわたり安定して再現できた。既に同じ手法でチェックボックス+正規表現+複数文字入力という長いシーケンスも問題なく完走したため、この2回の終了はモディファイアキー合成という、本セッション以前から3回異なる形で観測されている既知の環境不安定性(グローバル`keybd_event`によるフォーカス配送の失敗)によるものと結論した——コード上の欠陥ではない。

### 最終ゲート

Debug/Release/ubsan全1576/1576件green(3構成とも実行、Release/ubsanはサブエージェントへ委譲)。clang-tidy(変更・新規13ファイル個別実行)新規指摘0件——新規`find_dialog.cpp`も既存`find_replace_dialog.cpp`(クリーンな兄弟ファイル)と全く同じ出力形状(指摘0件)であることを確認。Release/ubsanとも警告0件(pre-existingな`D9025`ノイズのみ)、ubsanは`runtime error`/`AddressSanitizer`/`LeakSanitizer`を全ログでgrepし0件、新設した`find_dialog.cpp`/`normal_mode_wiring.cpp`のHWND/owner-windowプラミング・designated initializer構築に対する追加検出も無し。
...
## WI-25 — 自動整形(reindent)機能の新設(`edit.reindent`)

### 目的

WI-24完結後、ユーザーから「貴方の判断で次に改修する項目を決めて完成版のゴールを目指して欲しい」との指示があった。CLAUDE.mdが定める3つの正典ソースから次の候補を選ぶため一般探索エージェントへ完成度監査を委任した結果、**要件定義書§6「編集」に明記されている「自動整形」が、master_roadmap.mdの60機能マトリクスにも一度も登場せず、WI-01〜WI-24のどのWork Itemにも触れられておらず、`docs/issues/`にも起票されていない**ことが判明した——計画作成時に純粋に見落とされていたケース。既存の「自動インデント」(WI-12、Enterキーで前行のインデントを継承)とは別の機能で、既存テキストをブレースの深さに基づき再インデントする(Hidemaru/Sakura/VSCodeの「整形」「reindent」相当)。

副次的に「縦編集」の用語がmaster_roadmap.md内で無断に再定義されていた不整合も発見し、同日中に別issue([`master_roadmap_vertical_edit_terminology_drift.md`](../issues/master_roadmap_vertical_edit_terminology_drift.md))として解決した(ドキュメント修正のみ、本WIのコードには影響なし)。

続けてExplore agent・Plan agentへ設計を委任し、EnterPlanMode/ExitPlanModeで正式なPlan承認を得て実装した。

### 設計

汎用的な「ブレース深度」構造化APIはこのコードベースに一切存在しない(`syntax::`は意図的にtree-sitterを内部実装として隠蔽)が、既存の`syntax::parse(text, language)`(tree-sitterベースのトークナイザ)が返す`TokenKind::Punctuation`/`String`/`Comment`分類を再利用すれば、文字列リテラルやコメント内のブレースを正しく除外した精度の高いブレース深度計算を、生のtree-sitter APIに触れずに実現できると判明した。

**スコープ: 選択範囲対応。** 深さ計算は文書の先頭から対象行まで全トークンを辿る必要があり(選択範囲の有無に関わらず不可避)、選択範囲対応は同じ走査に「対象行集合に含まれる行だけ編集を出力する」フィルタを1つ追加するだけで実現できた。

**対応言語: 明示的な許可リスト。** `Python`(辞書/セットリテラルが`{}`を使うが主要ブロック構造はコロン+インデント)や`Shell`(関数本体は`{}`だが`if/fi`等の主要制御構文はキーワード対)は、ブレース文字が存在しても「自然にno-op」にはならず誤った再インデントを行うリスクがあるため、`app::supportsReindent(syntax::Language)`という明示的な許可リストを新設した——対応: `Cpp, C, JavaScript, Java, Go, Rust, TypeScript, Tsx, Php, Json, Css, PowerShell`(12言語)。言語未検出または非対応言語の場合はサイレントno-op(`dispatchGitRefreshDiffCommand()`の「静かにスコープを絞る」前例に倣う、能力の範囲外であり失敗ではないため)。

### 実装

新規`src/app/include/neomifes/app/reindent.h` + `src/app/reindent.cpp`。`supportsReindent()`(許可リスト判定)、`reindentSelectedLineRanges()`(カーソル選択→マージ済み行範囲、空=文書全体)、`computeReindentEdits()`(コア算法)を実装。1行に複数の`}`があっても表示上のディーデントは1段まで、というHidemaru/VSCode流の慣習(`} else {`が自分自身は1段ディーデントして表示されつつ、次の行は正しく元の深さへ戻る)を実装した。複数行コメント/文字列の内部行は完全に触れない設計。

**`computeReindentEdits()`の認知的複雑度がclang-tidyの閾値(25)を超過(49)したため、`DepthTracker`/`SkipRangeTracker`/`ScopeTracker`という3つの小さなヘルパークラスへ分解した。** 各クラスが「ブレースイベントを順に追ってdepthを追跡」「文字列/コメントの継続行判定」「選択範囲行判定」という単一の責務を持ち、メインループの分岐がフラットになった。

`editor_input.h`/`.cpp`へ`applyReindent()`を追加(既存`applyIndentationConversion()`と同じ「computeして空なら中断、カーソルスナップショット、`ReplaceAllCommand`ディスパッチ」の型)。`normal_mode_wiring.cpp`の`buildCommandRegistry()`へ`edit.reindent`(`CommandId::None`、パレット専用、既存`edit.convertTabsToSpaces`と同じ足跡)を登録。**ディスパッチロジックを`dispatchReindentCommand()`という独立関数へ切り出した。** ロジックをlambda内へ直接書くと`buildCommandRegistry()`自体の認知的複雑度が25→29へ超過することが判明したため(この巨大関数の複雑度予算が既にほぼ上限だったところへ2つのif分岐を追加したのが原因)、既存の`dispatchJsonFormatCommand()`と同じ「名前付き関数へ切り出しlambda本体を1行にする」パターンを踏襲して解消した。

### テスト作成

新規`tests/unit/app_reindent_test.cpp`(13件)。手計算で期待値を事前検証してから記述——特に「フラッシュレフトの入力では閉じブレース行が既に深さ0で正しいためno-op edit」という、当初の手計算では見落としていた挙動を、実際にテストを走らせて3件のテスト失敗から発見・修正した(実装ではなくテストの期待値側の誤りだった。既存`computeIndentationConversionEdits()`の「変更が無ければスキップ」慣習を正しく踏襲していることの再確認になった)。ネストしたブレース・`} else {`のディーデント/深度復帰・文字列内ブレースの除外・コメント内ブレースの除外・複数行コメントの内部行不変・tabs/spaces設定の反映・選択範囲なし=全文書/選択範囲あり=その行のみ・既に正しい入力=編集0件・空行不変・許可リストの網羅・行範囲マージ/境界規則、を検証。

### 実機ドッグフーディング

PowerShell + Win32 P/Invoke(WI-24で確立した「`keybd_event`でモディファイア状態のみ設定+`PostMessage`で対象コントロールへ直接キー配送」の確実な手法を踏襲)で以下を確認した: 意図的に崩したインデントを持つC++ファイルをコマンドパレット経由(Ctrl+Shift+P→"reindent"→Enter)で整形し、ネストしたブレース・文字列リテラル内のブレース(`"a { brace } inside a string"`)・`} else {`の深度復帰を含む全ての行が正しく再インデントされることをスクリーンショットで確認。Ctrl+Zで全変更が1回のUndoステップとして正しく巻き戻ることを確認。非対応言語(Python)のファイルで同じ操作を試み、タイトルバーの未保存マーカーが一切表示されない(内容変更なし)ことを確認しサイレントno-op動作を実証した。
...
## WI-26 — 縦編集(列編集)機能の完成(`column.append`)

### 目的

WI-25完結時に「次フェーズ候補として有力」と記録した「縦編集(列編集、MIFES由来)」に着手した。要件定義書§6に明記され、`docs/issues/master_roadmap_vertical_edit_terminology_drift.md`(WI-25で解決)により「縦書き入力」ではなく「MIFES流の列単位の追記/削除」を指すことは既に確定済みだった。

Explore agentへ2段階で調査を委任した結果、**当初の想定(`ColumnInsert/Delete/Overwrite/Append`の4専用コマンドが丸ごと未実装)は誤りで、実際のスコープは大幅に小さいと判明した**。矩形選択自体はPhase 4b8a/4b8gで実装済み(`SelectionModel::setRectangularSelection()`、特別な型ではなく行ごとに1個の通常`Cursor`の集合として表現)、描画も`RenderPipeline::drawSelectionsOnLine()`が「1行1カーソル」を透過的に描画するため専用コードは元々不要。**コード読解で確認した事実として、矩形選択への通常のタイプ入力(`handleChar`)・Delete/Backspace(`applyDeleteKey`)・貼り付け(`handlePaste`)は、いずれも既存の汎用マルチカーソル編集機構(`MultiCursorEditCommand`)がカーソルごとの選択状態を正しく個別処理しており、新規ロジックは一切不要だった。** 唯一「MIFESの縦編集の主用途」である行末への一括追記だけは、矩形選択の列位置と無関係に各行の実際の行末へ挿入する必要があり、既存の`SelectionModel::convertToLineEndCursors()`(Shift+Alt+Iとして実装済み)がその位置計算をそのまま提供する。

この調査結果をユーザーへ提示し、AskUserQuestionで実装方針(4コマンド対称実装 / 最小実装 / 実装追加なし)を確認した。**ユーザーは「最小実装」を選択**——新規コードは行末一括追記のコマンドパレット化のみとし、Insert/Delete/Overwriteは既存機構をそのまま使う。CLAUDE.mdの「不要な抽象化を避ける」方針とも合致する判断。

### 設計・実装

新規`ICommand`サブクラスも新規`app::`ヘルパー関数も作らず、`normal_mode_wiring.cpp`の`buildCommandRegistry()`へ`column.append`パレットコマンド1つを追加した。実体はShift+Alt+Iハンドラ(`handleSysKeyDownEvent()`)と全く同じ2行(`session.selection().convertToLineEndCursors(session.document())` + `syncRenderStateAndInvalidate()`)——既存キーの別名にパレット名と発見しやすさを与えるだけの、意図的に最小の追加。`keybindingLabel`は他のCommandId::Noneコマンド(通常は空文字列)と異なり、既存の固定ショートカット`Shift+Alt+I`を案内するため明示的に`u"Shift+Alt+I"`を設定した。

### テスト作成

`tests/unit/app_editor_input_test.cpp`へ5件の結合テストを追加(`RectangularSelectionColumnInsertTypesIndependentlyOnEachRow`/`...ColumnDeleteRemovesOnlyEachRowsOwnSpan`/`...ColumnOverwriteReplacesEachRowsSpanWithTypedChar`/`...ClampsShortRowToCollapsedCursorNoPadding`/`...ThenConvertToLineEndCursorsAppendsAtEachRowsRealEnd`)。`setRectangularSelection()`自身は`core_selection_model_test.cpp`で単体テスト済みだったが、それを実際の編集ハンドラ(`handleChar`/`handleKeyDown`)に通した結合シナリオは本WI以前は一切存在しなかった——「動くはず」ではなく実際に実行して確認することが本WIの核心の成果物。1件で手計算ミス(`setRectangularSelection()`の第2引数に誤った行の開始オフセットを使用)によるテスト失敗が発生、実装ではなくテスト側の誤りと確認して修正した(WI-25と同種の教訓の再確認)。

### ドキュメント同期

`docs/design/master_roadmap.md`(60機能マトリクス行・§22 U#21を解決済みへ更新)、`docs/design/detailed_design.md` §5.1.1(未実装だった4コマンド構想のコードスケッチを実際の最終設計に置き換え)、`docs/decisions/ADR-012-phase4a-editor-core-scope.md`(「Rendering Engineに選択範囲ハイライト描画が実装された時点」という再評価トリガーがPhase 4b8aで既に発火していたこと、本WIでの最終結論を注記)、`docs/user/keybindings.md`(新規「列編集(縦編集)」節、および矩形選択の開始ジェスチャが実際には`Alt`+クリックではなく`Shift+Alt`+クリックであるという既存の軽微な誤記の副次修正)を更新した。

### 実機ドッグフーディング

WI-24/WI-25で確立した「`keybd_event`でモディファイア状態のみ設定+`PostMessage`で対象コントロールへ直接キー配送」の手法(PowerShell + Win32 P/Invoke)で、4行の実データ("apple"/"banana"/"cherry"/"date")を開き、`Shift+Alt+↓`(キーボードのみ)で矩形選択を対話的に作成、以下4シナリオを全てスクリーンショットで確認した:

- **列挿入**(列幅0で3回`Shift+Alt+↓`→タイプ入力): 4行全てに同じ列位置へ独立に文字が挿入され、Ctrl+Zで1操作として巻き戻ることを確認
- **列削除**(`Shift+Alt+→`で幅2に拡張→`Delete`): 4行それぞれの選択範囲のみが削除されることを確認
- **列上書き**(同じ幅2の選択→タイプ入力): 4行それぞれの選択範囲が置換されることを確認
- **行末一括追記**(列2で矩形選択→コマンドパレットから「Column Append (Line End)」実行→タイプ入力): パレットの絞り込み結果に`Shift+Alt+I`のラベル付きで正しく表示されることを確認した上で実行、結果「apple!」「banana!」「cherry!」「date!」——矩形選択の列位置(列2)や各行の長さ(5/6/6/4文字)に関わらず、各行の実際の行末に正しく追記されることを確認。Ctrl+Zで1操作として巻き戻ることも確認

**`TIMELINE.md`の複数セッションにわたる既存の記録によれば、矩形選択の対話的作成自体はこれまで実機で一度も目視確認されていなかった(単体テストのみ)——本WIのドッグフーディングがこの積み残しを解消する初めての機会になった。**

**副次的発見: `rectangularAnchor`(矩形選択の基点)がキーボードのみの操作では一切リセットされないバグを発見した。** 列削除シナリオの検証中、直前の列挿入シナリオで設定された基点(列0)が、間に挟んだ`Ctrl+Home`・矢印キー移動・`Ctrl+Z`のいずれでもリセットされず、新しい矩形選択(列1開始のつもり)に無言で混入し、意図より1列分広い範囲が選択される事象を発見した。コード確認の結果、`rectangularAnchor`は`dispatchMouseDown()`内のプレーンなマウスクリックでのみリセットされ、キーボードのみの操作(移動・編集・Undo/Redo)では一切リセットされないと判明(`grep -rn rectangularAnchor src/app`で確認)。これはPhase 4b8g(既存・本WI以前)の潜在バグであり、WI-26のスコープ外と判断し実装修正はせず、[`rectangular_anchor_stale_across_keyboard_only_reuse.md`](../issues/rectangular_anchor_stale_across_keyboard_only_reuse.md)として起票した(プレーンなマウスクリックを挟んでリセットする回避策で、以後のシナリオは正しい選択範囲で検証できることを確認済み)。
...
## WI-31 — タイピング時に「全再描写されてガタつく」の原因特定(実測診断、コード変更なし)

### 目的

WI-28完了報告直後、ユーザーから新規報告:「まだキー入力すると全再描写されてウィンドウの描写がガタつく。」WI-28が対象としたステータスバー部分のチラつきとは別の訴えであり、メインのテキスト描画エリアを含む再描画そのものの体感問題。

### 調査

2件のExplore agentへ読み取り専用調査を委任し、`InvalidateRect`が全域invalidate(二重invalidateなし)であること、`RenderPipeline::render()`のフレームスキップがタイピング中は実質常に無効であること(ADR-011が意図的に採用した設計)、`Present1(1, 0, ...)`がSyncInterval=1(vsync同期、ティアリング無し)であることを確認した。あわせて`docs/history/TIMELINE.md`を横断調査し、Phase 3c以降ほぼ全WIで`--measure-frame`実測値が一貫して`avgFrameNs≈16.5ms`(60Hzの1vblank間隔そのもの)であること、既存ベンチ(`BM_TextLayoutCache_Miss`=542ns/行)からCPU側描画コストが無視できる規模であることを確認し、「フレーム時間はCPU負荷でなくvsync待ちに支配されている」という仮説を組み立てた。ただしタイピング固有のレイテンシ分布は未実測だったため、CLAUDE.mdルール10(性能改善はベンチマーク根拠必須)に従い実測診断を行った。

### 実測(一時計装、診断後に完全除去)

`RenderPipeline::render()`へ`platform::PerfClock`ベースの一時計装(`// DOGFOOD-TEMP`タグ)を追加し、実際にレンダリングされた各フレームの開始時刻・所要時間をCSVへ記録。Debugビルドへ確立済みハイブリッド技法(`keybd_event`+`PostMessage`)で2種類の速度のタイピングを注入した。

- **現実的な間隔(130ms/文字)**: フレーム間隔は注入間隔とほぼ一致(136〜152ms)、`renderOnce()`所要時間は0.54〜2.85ms——問題なし。
- **バースト間隔(20ms/文字)**: `renderOnce()`所要時間はCPU側は同水準(0.65〜2.05ms)だが、**フレーム開始間隔は注入した20msではなく一貫して30〜32ms(60Hzの1〜2vblank分)に張り付いた。** 26文字の注入は約520msで完了するが、25フレーム分の処理完了には約780ms要しており、入力到着速度がレンダリング処理速度を上回りWM_CHARがメッセージキューに滞留、表示が入力に追いつかず後追いになることを実測で確認した。

計装コードは診断後に完全除去(`grep`で残留無し確認、`git diff --stat`が空になることでrender_pipeline.cppがHEADと完全一致することを確認)。**このWIはコード変更を一切含まない**(調査・実測のみ)。

### 結論と次アクション

根本原因はCPUコストではなく、「1キー入力ごとに同期的にvsync待ちPresentを行う」というメッセージループ/描画パイプラインのアーキテクチャそのものにあると実測で裏付けられた。WI-28型(無駄な再構築への差分ガード追加)の修正では解決しない、より大きな設計変更が必要と判明したため、[`docs/issues/keystroke_burst_render_backlog.md`](../issues/keystroke_burst_render_backlog.md)(新規、P1)として詳細を記録し、対応方針(候補: 入力キューがドレインするまで再描画を遅延させる等)はCLAUDE.mdルール9(大規模変更は必ずユーザー承認)に従いユーザーへ提示、承認を得てからWI-32として設計・実装する。

### 検証

計装コード除去後、Debugビルドが警告無くクリーンにビルドできることを確認(`git diff --stat`が空、`render_pipeline.cpp`がWI-30コミット時点と完全一致)。コード変更が無いためRelease/ASan/UBSanの再検証・コミットは無し。
...
## WI-33 — 非同期シンタックストークン反映時の二重再描画を修正

### 目的

WI-32完了報告後、ユーザーから新規報告:「テキスト１行目入力時には依然として再描写が頻繁に発生して画面がカクつく。秀丸の快適さを超えるユーザビリティを実現せよ。」

### 調査

2件のExplore agentへ調査を委任。①WI-32の入力キューのドレイン優先は真の滞留がある場合のみ効き、通常速度のタイピングでは修正前と同じ「1キー入力=最低1vblankの同期Present」が発生する(1行目固有ではない構造的制約)。②`handleAppMessage()`の`kMsgSyntaxTokensReady`分岐が、WI-32のゲートを経由しない生の`InvalidateRect`を呼んでいると判明。`SyntaxWorker`は編集の度に文書全体を再走査するため、文書が短い(新規ファイル・1行目付近)ほど非同期処理が高速に完了し、編集自体の再描画とは別にほぼ毎キー入力ごとに2回目の独立した再描画が発生していた——これが「1行目で依然としてガタつく」の直接原因。`applyGitDiffReadyMessage()`/`applyLogIndexReadyMessage()`にも同型の生`InvalidateRect`があるが、いずれもユーザーの明示コマンドからのみ発火し毎編集での自動発火ではないため対象外と判断。

### 実装・設計上の教訓

`kMsgSyntaxTokensReady`ハンドラへ新規`requestPaintForBackgroundUpdate()`(`RenderPipeline::paintOverdue(50ms)`のみで判定)を適用。**当初はWI-32の`requestPaintUnlessBusy()`(キュー状態で判定)をそのまま再利用する設計で実装したが、実測ドッグフーディングで「通常速度のタイピングでは非同期トークン到着時もキューが空のため、修正が効かず二重描画が解消されない」ことを発見し、実装完了前に設計を修正した。** 背景ワーカー完了による再描画は次の実描画に乗せればよく緊急性が無いのに対し、ユーザー自身のキー入力による再描画は即時性が必要——この非対称性を反映し、判定基準を分離した(`paintOverdue()`のみを見る新規ヘルパー)。

### 検証

**実測ドッグフーディング:** 新規`.cpp`ファイルへ`"#include <iostream>"`(20文字)を70ms間隔でタイプ入力し、一時計装(`platform::PerfClock`、`DOGFOOD-TEMP`タグ、検証後完全除去)で実際の`RenderPipeline::render()`呼び出し回数を計測。**修正前(設計v1の誤り含む)は38回(≈2回/キー入力)、修正後(v2)は20回(1回/キー入力)に正確に収束することを確認。** スクリーンショットでシンタックスハイライト(言語検出「C++」)も正しく反映されることを確認。

Debug全1602/1602件green、clang-tidy新規指摘0件。Release/ASan/UBSan(clang-cl)は3構成ともサブエージェントへ委任・全green・サニタイザ診断0件・警告0件を確認。
...
Release/ASan/UBSan(clang-cl)3構成をサブエージェントへ委任、全構成1614/1614件green、サニタイザ診断0件、コンパイラ警告0件を確認(既知の非決定的`FrameMeasureTest.ProducesValidProfile`ハングも本回では再現せず正常pass)。

コミット: `24be0d0`。

---

## WI-35 — スクロールバーの内容依存の自動非表示を廃止(`SIF_DISABLENOSCROLL`)、「1行目でガタつく」の真因を特定・解消

### 目的

WI-34完了後、ユーザーから実機での切り分け結果の報告:「事象は水平スクロールバーをOFFにしたら発生しなくなった。水平・垂直スクロールバーはONの場合は常に表示する。文字数により表示非表示としない。」WI-31〜WI-34を通じて技術的原因を特定できないまま残っていた「1行目のみガタつく」の謎に対し、ユーザー自身が実機で行った経験的な切り分け(横スクロールバーを完全にOFFにすると症状が消える)によって、初めて再現性のある技術的手がかりが得られた。

### 調査

`syncHorizontalScrollBar()`/`syncVerticalScrollBar()`(WI-03/WI-34)のコードを読み直したところ、いずれも`SetScrollInfo()`呼び出し時に`SIF_DISABLENOSCROLL`フラグを一切指定していないと判明した(`grep -rn SIF_DISABLENOSCROLL src`は0件)。**Win32標準スクロールバーは、このフラグを指定しない限り、`SetScrollInfo()`で渡した範囲(`nMax`)がページサイズ(`nPage`)に収まると判断した瞬間、スクロールバー自体を自動的に非表示にする**(`ShowScrollBar(hwnd, SB_HORZ, FALSE)`相当の処理を内部で暗黙に行う)。`syncHorizontalScrollBar()`の`nMax`は`maxVisibleLineLength()`(現在行の文字数近似)、`nPage`は`visibleColumnCount()`(可視列数)であり、1行目に文字を打っていく過程でこの大小関係は頻繁に入れ替わる——**その度にネイティブスクロールバーが暗黙に消えたり現れたりし、クライアント領域の高さが変化してウィンドウ全体のレイアウトがズレていた。**

この機構は`RenderPipeline::resize()`を一切経由しない(Win32側が内部で完結させる処理のため)。**これが、WI-34で行った`resize()`呼び出し回数の実測でこの現象を検出できなかった理由そのものである。** 計装の対象が誤っていたのではなく、計装がそもそも届かない層でレイアウト変化が起きていた。

### 実機ドッグフーディング(検証を兼ねた事前確認)

修正前に、この仮説が実際に現象を説明するか実測で確認した。新規ファイルの1行目へ数字を1文字ずつ250文字連続入力し、各キー入力ごとに`GetClientRect`のクライアント高さを記録した。**修正前のビルドでは、250文字の入力中に高さが741px⇔724px(横スクロールバーの厚み分)の間で実に11回切り替わることを実測した。** これがユーザーの報告していた「ガタつき」の直接的な実体である。

### 実装

`syncHorizontalScrollBar()`/`syncVerticalScrollBar()`の`SCROLLINFO::fMask`へ`SIF_DISABLENOSCROLL`を追加。これにより、範囲がページ内に収まる場合もスクロールバー自体は非表示にならず、視覚的に無効化(グレーアウト)された状態で表示領域を維持し続ける——ユーザーの指示「ONの場合は常に表示する。文字数により表示非表示としない」を文字通り満たす。既存の明示的な`ShowScrollBar(FALSE)`呼び出し(ワードラップON時・ユーザー設定OFF時)はそのまま維持し、これらはユーザー/モード起因の意図的な非表示であるため区別した。修正中、隣接する`syncVerticalScrollBar()`の`kIntMax`定数宣言でclang-tidyの`modernize-use-auto`指摘(コメント行追加による行番号のずれで顕在化、WI-34由来の既存コード)を発見し、ついでに`auto`化して解消した。

### 検証

修正前後で同一のドッグフーディング手法により対照実験を行った: **修正後は同じ250文字入力で高さの遷移が0回**(741px始まりのまま完全に一定)であることを確認した(`git stash`で修正前後をビルドし直しての比較)。短い行での無効化(グレーアウト)表示もスクリーンショットで確認し、常に領域が確保されつつ内容が収まる場合は目立たない見た目になることを確認した。

Debug全1614/1614件green、clang-tidy新規指摘0件。Release/ASan/UBSan(clang-cl)3構成をサブエージェントへ委任、全構成1614/1614件green、警告0件(release限定のD9025コマンドラインメッセージとlibgit2のCMake非推奨警告は既存・無害)、サニタイザ診断0件を確認。既知の非決定的`FrameMeasureTest.ProducesValidProfile`ハングも本回では再現せず正常pass(5.47秒)。**なお検証委任時、3構成を並列実行するとリソース競合でASanのctestが無応答になる事象が発生したため、ASanのみ単独で再実行し完走を確認した——3構成の並列実行は今後避けるべき運用上の注意点として記録する。**
