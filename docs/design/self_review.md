# NeoMIFES 設計セルフレビュー v2.0

> 対象: [`CLAUDE.md`](../../CLAUDE.md) / [`basic_design.md`](basic_design.md) / [`detailed_design.md`](detailed_design.md) / [`docs/decisions/`](../decisions/)
> 初回レビュー日: 2026-07-14
> 最終更新日: **2026-08-04 (中間レビュー時、Phase 8f / 7y 完了時点)**
> レビュー観点: (A) 要件充足性 / (B) 設計整合性 / (C) 性能目標達成可能性 / (D) リスク / (E) 実装可能性
>
> 🔴 **現在の状態を知るには §J (v2.0 中間レビュー、本ファイル末尾) を読むこと。**
> ⚠️ **§A〜§I は歴史的記録として凍結されている。**
> - §A〜§G: Phase 0 (要件確認・初回設計) 時点のスナップショット
> - §G'/§H/§I': **v1.6 (Phase 2b/3b 完了時、2026-07-16) のスナップショット。以後 20 フェーズ更新されていない** — 内容は当時の判断根拠を追うための一次資料としてのみ有効
>
> **本ファイルの運用に関する反省 (2026-08-04):** v1.7 で「サブフェーズ粒度の記録は RESUME_HERE.md/TIMELINE.md に委ね、本ファイルはフェーズ境界の包括レビューのみを担当する」と整理したが、**その後フェーズ境界 (Phase 4 完了 / 5 完了 / 6 完了 / 7 完了 / 8 進行中) で一度も包括レビューが行われなかった。** 結果として「エンジンだけが進み製品が完成しない」という構造的問題を検知する機会が 5 回失われた。**以後、親フェーズ完了時には必ず本ファイルへ包括レビュー節を追加する** (CLAUDE.md §11)。
...
## J. v2.0 中間レビュー (2026-08-04、Phase 8f / 7y 完了時点)

> ~~**本節が現在の状態を示す唯一の節である。**~~ **(2026-09-11 追記) 本節自体、M2〜M5 達成・WI-46・プロジェクト凍結を経て約5週間分古くなっている。** §J に続く §K (M2以降の包括レビュー) は結局追加されなかった——本ファイル自身が J.4 で挙げていた「フェーズ境界で包括レビューを行う」というルールが、皮肉にもこの節以降は一度も実行されなかったことになる。**現在の状態を知るには [`docs/phase_reports/project_freeze_2026-09-11.md`](../phase_reports/project_freeze_2026-09-11.md) を読むこと。** §G'/§H/§I' は v1.6 (2026-07-16) で凍結された歴史的記録、本節 §J も 2026-08-04 時点のスナップショットとして同様に凍結保存する。
> 詳細な分析・証拠・是正計画は [`gap_analysis.md`](gap_analysis.md) を参照 (本節はその要約、gap_analysis.md 自体も同様に凍結・現在地は同レポートへ誘導される)。

### J.1 総合評価

**技術的実装品質は世界水準。ただし「製品」として見たときの優先順位づけが根本的に誤っていた。**

| 観点 | 評価 |
|---|---|
| エンジン層 (Document/Rendering/Search/Encoding/Syntax/Plugin) | 🟢 **商用水準** — 全ての性能 DoD を実測達成 |
| アプリケーションシェル (保存/タブ/設定/IME/クローム) | 🔴 **ほぼゼロ** — **編集内容をファイルに保存できない** |
| 開発規律 (ADR 21 本 / 966 テスト / 実機 probe 文化 / 失敗の正直な記録) | 🟢 **商用開発組織の水準を上回る** |
| ロードマップの網羅性 | 🔴 **構造的欠陥** — L6 (Application Shell) にフェーズが割り当てられていなかった |
| ドキュメント鮮度 | 🟡 **部分的に劣化** — README が 8 フェーズ分、self_review が 20 フェーズ分、issue ヘッダが 3 週間分 陳腐化していた |

**§G' (v1.6) からの最大の変化:** 当時「要件カバレッジ 100% (設計レベル)」と評価したが、これは**設計書レベルでの網羅性**を見たものだった。今回**実コードに対する機械的検証**を行った結果、要件定義書 §6「編集」の必須機能 21 項目のうち**達成は 7 項目 (33%)** であり、うち 3 項目は保存機能の実装で自動的に解消する。「設計に書いてある」と「実装されている」の乖離を測る観点が v1.6 の評価軸に欠けていた。

### J.2 残リスク一覧 (v2.0、§H を置き換え)

| # | リスク | 深刻度 | 状態 |
|---|---|---|---|
| **R15 (NEW)** | **文書保存機能の欠落** | **最高 (P0)** | 🔴 **未解消**。Phase 8.5a で対応。[`no_document_save_capability.md`](../issues/no_document_save_capability.md) |
| **R16 (NEW)** | **アプリケーションシェル全般の欠落** (開く/タブ/メニュー/ステータスバー/行番号/横スクロール/リソース) | **最高 (P0)** | 🔴 **未解消**。Phase 8.5b〜8.5g。[`no_application_shell.md`](../issues/no_application_shell.md) |
| **R17 (NEW)** | **メインエディタの IME 未対応** — 日本語市場で単独で出荷を阻む | **最高 (P0)** | 🔴 **未解消**。Phase 8.5e。[`no_ime_support_in_main_editor.md`](../issues/no_ime_support_in_main_editor.md) |
| **R18 (NEW)** | **設定システムの欠落** — 13 箇所で機能縮退の理由になり、`kTabWidth` の二重定義まで発生 | 高 (P1) | 🔴 **未解消**。Phase 8.6a。[`no_settings_system.md`](../issues/no_settings_system.md) |
| **R19 (NEW)** | **`main.cpp` 2,053 行の肥大化** — タブ追加時に確実に破綻する | 高 (P1) | 🔴 **未解消**。Phase 8.5c で `EditorSession`/`Workspace` へ解体 |
| **R20 (NEW)** | **上位設計書を要約した図から項目が脱落し、下位の作業計画に反映されない** — L6 が CLAUDE.md §3 の簡略図で消え、roadmap のフェーズ表がそれに追従した | 高 | 🟡 **一度顕在化・修正**。CLAUDE.md §3 に L6 を復活させた。**要約は情報を落とすという当たり前の事実が、最も影響力のある図で起きると致命的になる** |
| **R21 (NEW)** | **完了判定がフェーズ計画書に閉じており、プロダクト全体要求への照合が無い** | 高 | 🟡 **対策済み**。CLAUDE.md §11 に照合項目を追加。Phase 4b8 が実際にこの誤りを犯した (自動インデント・縦編集が未実装のまま「完全に完了」宣言) |
| **R22 (NEW)** | **実アプリ検証が「プロセスが 3 秒後も生存」に縮退していた** | 高 | 🟡 **対策済み**。CLAUDE.md §11 に「ドッグフーディング DoD」「生存確認は機能確認ではない」を明記 |
| R6 | AI プラグインの API キー漏洩・プロセス分離の是非 | 中 | 未着手 (Phase 9、v2.1 で最後尾へ移動) |
| R12 | LineIndex の O(log n) 化不可能 | 低 | 🟡 仕様として受容。Phase 7p でインクリメンタル更新は実装済み |
| **R23 (NEW)** | **大規模文書の増分解析 DoD 未達** (50 万行 155.95ms、目標 ≤50ms) | 低 (P2) | 🧊 **凍結**。4 フェーズ挑戦し tree-sitter の構造的限界と結論。Phase 12 直前に「達成」か「DoD 改訂」かを判断 |
| R1 / R10 / R11 / R13 / R14 | (v1.6 時点の項目) | — | 🟢 全て解消済み。§H 参照 |

### J.3 次アクション (v2.0、§I' を置き換え)

1. ⏭️ **Phase 8.5a — 文書保存基盤** (最優先)。roadmap §8.5.3 に設計方針を規定済み。未決事項 U#22 / U#23 を実機 probe で検証してから実装する
2. ⏭️ Phase 8.5b〜8.5g — ファイル UI / `main.cpp` 解体 / タブ / IME / クローム / 横スクロール
3. ⏭️ Phase 8.6a〜8.6e — 設定 / キーバインド / テーマ / 自動保存 / 基本編集の穴埋め
4. ⏭️ **Phase 12' — MVP 出荷判定** (新設)。ここで一度「売れる状態」を作り、実ユーザーの反応を後続へ反映する
5. 🧊 Phase 9 (AI) / 8g (AppContainer) / 7z (大規模文書 DoD) は凍結

### J.4 本レビューで維持すべきと確認した強み

批判的分析だけを残すと、次のセッションが**本当に優れている部分まで作り直しかねない**ため明記する:

- **エンジン層の設計と実装は変更する必要がない。** Piece Tree + mmap + Lazy Decode、Direct2D レンダリング、tree-sitter 増分解析、RE2 検索、C ABI プラグイン境界 — いずれも VSCode 同等以上の水準にある
- **ADR 文化と実機 probe 規律は継続する。** tree-sitter のノード型を記憶で書かず毎回 probe で実測してから実装する規律は Phase 7a 以来一貫しており、実際に多くのバグを実装前に防いでいる
- **失敗を正直に記録する文化は極めて価値が高い。** Phase 7u の「実装完了後に全面 revert」、DoD 未達を「未達」と書き続けた記録は、本レビューが構造的問題を特定できた最大の材料だった

**問題は規律の欠如ではなく、規律が向けられた方向の誤りである。**
