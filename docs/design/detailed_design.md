...
**Phase 4a (2026-07-16) で実装確定 (ADR-012)。** 本節は Phase 0 時点のスケッチから、`src/core/` の実装内容に同期済み。§5.1.1 (縦編集)・§5.2 の `FoldingMap` は ADR-012 により Phase 4b 以降へ明示的に延期 (削除はしない)。**Phase 4b1 (2026-07-17) でキーボード入力配線 (`src/app/editor_input.h/.cpp`)・キャレット描画・マウスホイールスクロールを実装、Phase 4b2 (同日) でマウスクリック位置特定・選択範囲ハイライト描画、Phase 4b3 (同日) でドラッグ選択、Phase 4b4 (同日) でダブルクリック単語選択・トリプルクリック行選択を実装**(複数カーソルは Phase 4b5 へ延期、詳細は本節末尾および §6 参照)。
...
### 5.3 入力配線・キャレット・選択・ドラッグ・単語/行選択 (Phase 4b1〜4b4、2026-07-17実装)
...
**Phase 4b5 へ延期:** Alt+クリックによる複数カーソル追加(編集コマンドの複数カーソル対応も併せて必要)、選択範囲のクリップボードコピー、ダブルクリック→ドラッグでの単語単位ドラッグ拡張(現状は文字単位ドラッグにフォールバック)、`WM_CAPTURECHANGED` の明示的ハンドリング、ドラッグ中のウィンドウ端オートスクロール、PageUp/PageDown、Ctrl+矢印(`MovementUnit`未実装のため)。
