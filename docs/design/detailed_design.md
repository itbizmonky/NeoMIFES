...
- **複数カーソル操作**: `moveAll`/`moveAllTo`/`setCursors` が全カーソルに適用され、適用後に `mergeOverlapping()` で範囲重複/同一位置のカーソルを1つにマージする(`isPrimary` は OR で伝播)。
...
- **`MovementUnit`(単語/段落単位移動、矢印キー等でのジャンプ)は Phase 4b5b 時点でも未実装** — `selectWordAt`/`selectLineAt` (ダブル/トリプルクリック用) とは別に、Ctrl+矢印等の移動コマンド向けの単語境界移動は未着手。ADR-012 参照。
- **矩形選択 (`setRectangular`) は Phase 4b5b 時点でも未実装** — §5.1.1 参照。
...
### 5.3 入力配線・キャレット・選択・ドラッグ・単語/行選択・複数カーソル (Phase 4b1〜4b5b、2026-07-17実装)
...
**スコープ外 (Phase 4b6以降へ延期):** Alt+Shift+クリック(新カーソルの選択範囲拡張)・Alt+ドラッグでの追加カーソルの選択拡張、選択範囲のクリップボードコピー、ダブルクリック→ドラッグでの単語単位ドラッグ拡張(現状は文字単位ドラッグにフォールバック)、`WM_CAPTURECHANGED` の明示的ハンドリング、ドラッグ中のウィンドウ端オートスクロール、PageUp/PageDown、Ctrl+矢印(`MovementUnit`未実装のため)。
