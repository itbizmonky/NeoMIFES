...
### 5.3 入力配線・キャレット・選択ハイライト・ドラッグ選択 (Phase 4b1/4b2/4b3、2026-07-17実装)
...
**Phase 4b4 へ延期:** ダブルクリック(単語選択、単語境界判定の仕様についてユーザー合意が必要 — ADR-012 の既存トリガー)・トリプルクリック(行選択)、Alt+クリックによる複数カーソル追加(編集コマンドの複数カーソル対応も併せて必要)、選択範囲のクリップボードコピー、`WM_CAPTURECHANGED` の明示的ハンドリング、ドラッグ中のウィンドウ端オートスクロール、PageUp/PageDown、Ctrl+矢印(`MovementUnit`未実装のため)。
