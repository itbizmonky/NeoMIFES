...
**Phase 4a (2026-07-16) で実装確定 (ADR-012)。** 本節は Phase 0 時点のスケッチから、`src/core/` の実装内容に同期済み。§5.1.1 (縦編集)・§5.2 の `FoldingMap` は ADR-012 により Phase 4b 以降へ明示的に延期 (削除はしない)。**Phase 4b1 (2026-07-17) でキーボード入力配線 (`src/app/editor_input.h/.cpp`)・キャレット描画・マウスホイールスクロールを実装** (マウスクリック位置特定・選択範囲ハイライトは Phase 4b2 へ延期、詳細は本節末尾および §6 参照)。
...
### 5.3 入力配線とキャレット描画 (Phase 4b1、2026-07-17実装)
...
**Phase 4b2 へ延期:** マウスクリックでのカーソル位置特定 (`WM_LBUTTONDOWN` + `IDWriteTextLayout::HitTestPoint` — このコードベースに前例なし)、選択範囲のハイライト描画、複数カーソルの入力経路(Alt+Click等)、PageUp/PageDown、Ctrl+矢印(`MovementUnit`未実装のため)。
