...
## 設計指針 (詳細設計 §3.1 + [ADR-006](../decisions/ADR-006-piece-tree-implementation.md))

**実装形態は [ADR-006](../decisions/ADR-006-piece-tree-implementation.md) で "Path-Copying Persistent RB-Tree" に確定。**

- 各ノードが `Piece` を保持し、**`std::shared_ptr<const Node>`** で immutable に保持
- サブツリーの `length` と `newlineCount` を集約 (順序統計)
- 挿入/削除は **変更経路上の O(log n) 個のノードを新規に確保** して RB 回転で平衡維持
- ルートは `std::atomic<std::shared_ptr<const Node>>`。snapshot() は atomic load だけで O(1)
- リーダは古いルートを掴んだままで安全 (共有部分は immutable、変更部分は別ノード)
