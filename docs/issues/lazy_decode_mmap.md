...
- **優先度:** ~~高 (10GB × 20MB を両立させる要件)~~ → **🟢 実質解消 (2026-07-15、Phase 2b3)**。下記「完了条件」は 1 項目 (mmap ビュー LRU 追い出し) を除き全て達成済みで、その 1 項目も「単一ビュー設計へ変更したため対象外」と結論済み。**優先度表記が 2026-08-04 の中間レビューまで「高」のまま放置されていた** — issue ヘッダの鮮度も CLAUDE.md §11 の点検対象に含めること
