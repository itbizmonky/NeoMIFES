...
| [`--measure-frame`が`ubsan`(clang-cl)ビルドでのみハングする](frame_measure_hangs_under_ubsan_clang_cl.md) | `FrameMeasureTest.ProducesValidProfile`が`release`/`asan`では通るが`ubsan`でのみタイムアウト。WI-29の検証中に発見、WI-29より前から存在する潜在バグと切り分け済み(サニタイザ診断は検出されず単純なハング) | 待機 (原因未調査、`--measure-frame`は開発者向けフラグで製品機能への影響なし) |
