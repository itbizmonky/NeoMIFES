...
| [横スクロールバーのつまみドラッグが`WM_HSCROLL`の16bit値をそのまま使っている](hscroll_thumb_drag_16bit_truncation.md) | 列65535を超える長い1行を持つファイルでつまみドラッグが破綻する。WI-34で新設した縦スクロールバーは`GetScrollInfo(SIF_TRACKPOS)`で回避済み、既存の横スクロールバー(WI-03)は未修正 | 待機 (WI-34発見、実用上は極めて長い1行を持つファイルでのみ顕在化) |
