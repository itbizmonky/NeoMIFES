...
| Issue | 概要 | 対応 Phase |
|---|---|---|
| [バーストタイピング時、vsync同期Presentのメッセージキュー滞留により再描画が「ガタつく」](keystroke_burst_render_backlog.md) | WI-31実測: CPU側描画コストは常に3ms未満だが、20ms間隔のバースト入力ではフレーム開始間隔が30〜32ms(1〜2vblank)に張り付き、WM_CHARがキューに滞留し表示が入力に追いつかない。対応方針はユーザー承認待ち | 未定 (WI-32、方針未承認) |
