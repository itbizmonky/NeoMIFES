...
| [バーストタイピング時、vsync同期Presentのメッセージキュー滞留により再描画が「ガタつく」](keystroke_burst_render_backlog.md) | 🟢 WI-32 (2026-09-06)。`syncRenderStateAndInvalidate()`へ`PeekMessageW(WM_KEYFIRST..WM_KEYLAST, PM_NOREMOVE)`ベースの入力キューのドレイン優先を実装、50ms時間ベースの安全弁(`RenderPipeline::paintOverdue()`)付き。実測で23文字の真のバースト入力が23回→1回の再描画へ集約されることを確認。検証中に副次的発見1件を[`uniform_interval_typing_frame_cadence_unexplained.md`](uniform_interval_typing_frame_cadence_unexplained.md)として別途起票(実害軽微、P2) |
