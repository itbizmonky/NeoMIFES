...
| [`--measure-frame`がウィンドウ非表示(SW_HIDE)状態だと無期限にハングする](measure_frame_hangs_forever_on_hidden_window.md) | Release/UBSan双方で100%再現確認済み。`Present1(1,0,...)`(vsync同期)がウィンドウの合成状態に依存してブロックすると推定、`frame_measure_hangs_under_ubsan_clang_cl.md`調査中に発見 | 待機 (次点候補。実害は非表示ウィンドウから起動する自動化シナリオに限定) |
