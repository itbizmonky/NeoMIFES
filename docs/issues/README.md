...
| [`Viewport::setVisibleLineCount()`が本番コードから一度も呼ばれずPageUp/PageDownが実質0行しか移動しない](viewport_visible_line_count_never_set_pageup_pagedown_noop.md) | 実機で再現確認済み。WI-03の水平方向の配線(`RenderPipeline::visibleColumnCount()`→`Viewport::setVisibleColumnCount()`)に相当する縦方向の配線が一度も実装されなかった | 待機 (WI-34発見、次点候補) |
