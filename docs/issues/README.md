...
| [`--measure-frame`がウィンドウ非表示(SW_HIDE)状態だと無期限にハングする](measure_frame_hangs_forever_on_hidden_window.md) | 🟡 WI-41 (2026-09-10) で真因を確定。Win32の`STARTF_USESHOWWINDOW`仕様上、外部ランチャーが`SW_HIDE`を設定するとアプリ自身の`ShowWindow(SW_SHOWNORMAL)`が上書きされる。`MainWindow::create()`のShowWindow/UpdateWindow呼び出しは全モード共通のため理論上は通常起動モードも影響を受けうる(未検証)と判明、対応方針(1:起動完了待ちガード追加/2:タイムアウト機構追加/3:見送り)はユーザー判断待ち | 待機 (対応方針の決定待ち、実際の発生報告は無し) |
