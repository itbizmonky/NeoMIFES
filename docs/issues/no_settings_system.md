...
- [ ] `core::Settings` が実装され、`%APPDATA%\NeoMIFES\settings.json` から読み書きできる
- [ ] **既存のハードコード定数の移行を完了している** — 特に `kTabWidth` の二重定義を解消する。移行せずに設定システムだけ作ると負債が残る
- [ ] 設定ファイルが壊れている / 存在しない場合、既定値で安全に起動する
- [ ] 設定変更が再起動なしで反映される (最低限: フォント / タブ幅 / テーマ)
- [ ] 単体テストで「読み込み → 変更 → 保存 → 再読み込み」のラウンドトリップを検証している
