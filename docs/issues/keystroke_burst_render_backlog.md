# Issue: バーストタイピング時、vsync同期Presentのメッセージキュー滞留により再描画が「ガタつく」(✅ WI-32+WI-33で解決、残課題1件あり)

- **起票日:** 2026-09-05(WI-31、ユーザー報告「まだキー入力すると全再描写されてウィンドウの描写がガタつく」の実測診断)
- **解決日:** 2026-09-06(WI-32、入力キューのドレイン優先を実装。同日WI-33で「1行目入力時に依然としてガタつく」との追加報告に対応、非同期シンタックストークン反映の二重描画を修正。①の構造的制約〔通常速度タイピングでの1vblank/キー入力〕は残課題として記録)
...
## WI-33での追加解決内容(2026-09-06、「1行目入力時に依然としてガタつく」報告への対応)

WI-32完了後、ユーザーから「テキスト１行目入力時には依然として再描写が頻繁に発生して画面がカクつく」との報告。2件のExplore agentへ調査を委任し、以下の2点を確認した。

**① WI-32の適用範囲の限界(1行目固有ではない、既知の制約として記録):** WI-32の入力キューのドレイン優先は、メッセージキューに真の滞留が生じた場合のみ効く。通常の(極端なバーストではない)速いタイピングでは、1つ前のキー入力の処理がPresent1のvsync待ちを含めて完了してから次のキー入力が物理的に到着するため、キューには何も残らず、修正前と同じく**1キー入力ごとに最低1vblank(≈16.6ms)の同期Present待ちが発生する**。これは文書内のどこでタイプしても同じ、構造的な制約(下記「残課題」参照)。

**② 決定的な原因(1行目/新規ファイル固有、本WIで修正):** `handleAppMessage()`の`kMsgSyntaxTokensReady`分岐(非同期シンタックスハイライト完了時のコールバック)が、WI-32のゲートを一切経由しない生の`::InvalidateRect`を呼んでいた。`SyntaxWorker`は編集の度に**文書全体**を再走査するため、**文書が短い(新規ファイルや1行目付近)ほどこの非同期処理が高速に完了し、編集自体の再描画とは別に、ほぼ毎キー入力ごとに2回目の独立した再描画が発生していた**。大きな文書ではこの完了が次の編集まで遅延し自然に合流するため目立たない。

**修正:** `kMsgSyntaxTokensReady`ハンドラに新規`requestPaintForBackgroundUpdate()`(`RenderPipeline::paintOverdue(50ms)`のみで判定、キューの状態は見ない)を適用。**設計上の教訓:** 当初WI-32の`requestPaintUnlessBusy()`(キュー状態で判定)をそのまま再利用しようとしたが、実測ドッグフーディングで「通常速度のタイピングでは非同期トークン到着時もキューが空のため、修正が効かず二重描画が解消されない」ことを発見・特定した。背景ワーカー完了による再描画は「次の実描画に乗せればよい、緊急性が無い」ものであり、ユーザー自身のキー入力による再描画(即時性が必要)とは判定基準を分けるべきだった——安易に既存ヘルパーを使い回さず、実測で検証したことで設計ミスを実装完了前に発見できた。

**実測:** 新規`.cpp`ファイルに`"#include <iostream>"`(20文字)を70ms間隔でタイプ入力し、実際の`RenderPipeline::render()`呼び出し回数を計測。修正前は38回(ほぼ2回/キー入力)だったものが、修正後は**20回(1回/キー入力)**に正確に収束することを確認した。スクリーンショットでシンタックスハイライト(言語検出「C++」)も正しく反映されることを確認。

Debug/Release/ASan/UBSan(clang-cl)全1602/1602件green、clang-tidy新規指摘0件。

## 残課題(WI-33でも未解決、①の構造的制約)

メッセージキューに滞留が無い通常速度のタイピングでも、1キー入力ごとに最低1vblankの同期Present待ちが発生するという、より根本的なアーキテクチャ上の制約は未解決のまま。解消にはSyncInterval=0への変更(ティアリングとのトレードオフ)や描画スレッド分離といった、ADR-011を覆すレベルの大規模変更を要する。WI-33の修正適用後もなお「もっさり感」が残るようであれば、この残課題を次のWIとしてユーザーへ改めて提示する。
