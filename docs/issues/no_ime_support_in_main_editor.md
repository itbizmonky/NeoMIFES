...
- [ ] メインエディタで未確定文字列が下線付きインライン表示される
- [ ] 変換対象節がハイライトされる
- [ ] 候補ウィンドウがキャレット位置に追従する
- [ ] 変換確定後、確定文字列が正しく `Document` へ挿入される (Undo 1 ステップ)
- [ ] 複数カーソル時の挙動が定義されている (最低限: プライマリカーソルのみで変換、または IME 中は複数カーソルを畳む)
- [ ] **実機で MS-IME による手動確認を完了している** (スクリーンショット等の証跡を TIMELINE.md に記録)
- [ ] 中国語 / 韓国語 IME の確認は Phase 12 へ (本 issue のスコープ外)
