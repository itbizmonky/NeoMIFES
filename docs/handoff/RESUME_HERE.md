...
| **Phase 4 (Editor Core: Cursor/SelectionModel/Command/Undo/Viewport)** | ⏭️ **次回着手** |
...
- Issue (Phase 2b/3c 引継ぎ): [`docs/issues/`](../issues/)
- フェーズ報告:
...
Phase 4 (Editor Core: Cursor/SelectionModel/Command/Undo/Viewport) に着手せよ。
着手前に docs/phase_reports/phase_3_report.md §6 と detailed_design.md §5 を確認すること。
```
...
**Phase 4 (Editor Core) 着手時に確認すること:**
1. `docs/design/detailed_design.md` §5 (Editor Core 詳細) の `Cursor`/`SelectionModel`/`ColumnInsertCommand` 等のスケッチを起点にする。Phase 0 時点の設計であり、Document/Render 両エンジンの実装確定後の再検証が必要
2. `RenderPipeline::setTopLine()` (`src/render/include/neomifes/render/render_pipeline.h`) は既存の Phase 4 向けフック — `--measure-frame` ハーネスが現状唯一の呼び出し元。対話的スクロール入力の実装でこれを実際に駆動する `Viewport` クラスへの置換を検討する
3. 対話的な1行単位編集が実現したら、[ADR-011](../decisions/ADR-011-phase3c-render-cache-scope.md) の再評価トリガーに従い細粒度 DamageTracker の要否を判断する
4. CLAUDE.md §7 の Phase 4 DoD は「100万Undo達成」— 計測手法は Phase 1/3c の `--measure-startup`/`--measure-frame` パターンを踏襲できるか検討する
