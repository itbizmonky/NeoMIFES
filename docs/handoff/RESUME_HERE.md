...
> **次にやること: 🎉 M1 完全なドッグフーディング (ユーザー実施、編集→保存→コミットの一連) → WI-03 (横スクロール)** — `build_plan.md` §5 参照。WI-01 (文書保存基盤) は完了・コミット済み (`a4a0445`)。WI-02 (ファイルライフサイクル UI) は実装・コミット済み (`3e611d8`)。ユーザーが実際にドッグフーディングを試み、2件の実害あるバグ (Ctrl+O後の画面未反映/マウスホイールEOF超過スクロール) を発見・報告 → 根本原因を特定・修正・回帰テストで実証、コミット済み (`5712435`)。**ユーザーが2バグとも解消したことを再確認済み (2026-08-05)。ただしこれは2バグの再現手順のみの確認であり、「NeoMIFES自身のソースを開いて編集・保存・実際にコミットする」という完全なドッグフーディングループはまだ未実施 — 🎉 M1は引き続き未達成のまま。** §3.69参照。ユーザーへ完全なドッグフーディングの実施を依頼中。
>
> ---
...
>
> ### 次にやること
> **Phase 8.5a (文書保存基盤) は完了した (2026-08-04、WI-01)。Phase 8.5b (WI-02、ファイルライフサイクル UI) も実装・自動テスト・ローカルビルド検証まで完了した (2026-08-04、§3.68参照)。** ユーザーが実際にドッグフーディングを試み、2件の実害あるバグ (Ctrl+O後の画面未反映/マウスホイールEOF超過スクロール) を発見。両方とも修正・回帰テストで実証済み (2026-08-05、§3.69参照)、ユーザーへ再確認を依頼中 — 完了確認後、次は **Phase 8.5g (WI-03、横スクロール)**。着手前probeで「mmap解放は不要」(U#22/U#26解消)・「エラーコードではなく実ファイル存在チェックでリカバリ判断」(U#23解消) と判明し、`build_plan.md`/roadmap原案の一部を意図的に簡略化した — 詳細は §3.67 参照。
>
> ### 新設・更新した文書
...
| **8.5b** | **ファイルライフサイクル UI** (Ctrl+S/O/N、`IFileDialog`、D&D、未保存警告) | 🟡 **実装完了 (WI-02、コミット済み`3e611d8`、§3.68参照)。ドッグフーディングで2件のバグ発覚→修正・ユーザー再確認済み (§3.69参照)、コミット`5712435`/`8199c38`、pushはユーザー指示待ち。🎉 M1は完全なドッグフーディング(編集→保存→コミット)がまだ未実施のため引き続き未達成** |
...
- [ ] 🎉 **完全なドッグフーディング(NeoMIFES自身のソースをNeoMIFESで開いて編集・保存・実際にコミット)** ← **上記の確認とは別物。ユーザーへ改めて質問し「2件のバグ修正のみ確認、編集・保存・コミットまではまだ試していない」と明確な回答を得た。未実施のまま**

**コミット済み`5712435`/`8199c38`、pushはユーザーの明示指示待ち。** ユーザーが実際にNeoMIFES自身のソースを編集・保存・コミットできたことを確認できたら、🎉 M1達成を正式に記録し、次はWI-03(横スクロール)。
...
```
RESUME_HERE.md §3.69 (WI-02ドッグフーディングで発覚した2バグの修正) を
読んで現状を把握せよ。

WI-01/WI-02は完了・コミット済み。ユーザーが実際にドッグフーディングを
試み、2件の実害あるバグ (Ctrl+O後の画面未反映/マウスホイールEOF超過
スクロール) を発見・報告した。両方とも根本原因を特定し修正・回帰テスト
で実証済み (Debug/Release/ubsan全1002/1002 green)、コミット済み
(`5712435`/`8199c38`)。ユーザーへ再確認を依頼したところ「2件のバグ修正
のみ確認、編集・保存・コミットまではまだ試していない」との回答を得た
(2026-08-05)。

🎉 M1の完了条件そのもの(NeoMIFES自身のソースをNeoMIFESで開いて編集・
保存・実際にコミットする、完全なドッグフーディングループ)はまだ未実施
のまま。ユーザーへ改めてこの完全なループの実施を依頼し、確認が得られ
たら、build_plan.md §3 の WI-02 行にコミットハッシュを記入し M1 達成を
正式に記録した上で、WI-03 (横スクロール、build_plan.md §5 参照) に
着手すること。
