...
> **最終更新:** 2026-07-19 (Phase 5b3a・5b3b・5b3c・4b8a 完了、push待ち)
...
| Phase 4b8a (矩形選択、基本機能) | ✅ 完了 (未push、保留解除は§3.22参照) |
...
| Phase 5b3a (Find bar UI基盤: WC_EDIT子コントロール・インクリメンタル検索・マッチハイライト・F3ナビゲーション) | ✅ 完了 (未push) |
| Phase 5b3b (置換行配線 Ctrl+H: Replace edit・Enter/Ctrl+Enter・FindReplaceState統合) | ✅ 完了 (未push) |
| Phase 5b3c (コマンドパレット Ctrl+Shift+P: CommandPalette・ファジー検索・6コマンド登録) | ✅ 完了 (未push) |
| **Phase 5c (Grep等) または Phase 4b8b以降 (タブ変換・N対N分配・フリーカーソル等) 再開** | ⏭️ **次回選択** |
...
Phase 5b3a/5b3b/5b3cで完了済み、Phase 4b8のうち矩形選択の基本機能(4b8a)も完了済み。
次は以下いずれかをユーザーに選択させてから Plan Mode で詳細設計を起こすこと:
1. Phase 5c (Grep / 複数フォルダ検索 / 検索履歴 / タグジャンプ、master_roadmap.md §5.5)
2. Phase 4b8の残り (タブ⇔スペース変換・N対N分配クリップボード・フリーカーソル・マーカー・
   桁位置ジャンプ・Alt+Shift+矢印でのキーボード矩形選択拡張、master_roadmap.md §3.7のスコープ外一覧参照)
着手前に本ファイル §3.19/§3.20/§3.21/§3.22 末尾のスコープ外一覧を読むこと。まだ実施していない
実アプリでのCtrl+F/Ctrl+H/Ctrl+Shift+P/Shift+Alt+ドラッグ(矩形選択)/日本語IME視覚確認があれば、
このセッションの冒頭でユーザーに依頼すること(§3.19/§3.20/§3.21/§3.22参照)。
```
...
**Phase 5b3b (次回、置換行配線 Ctrl+H) 着手時に確認すること:**
1. `detailed_design.md` §7.1''''(Phase 5b3a実装済みの`ui::FindBar`/`MatchVisual`)を先に読む — `currentMatches`/`currentMatchIndex`は既に`main.cpp`側にローカル状態として存在するため、5b3bはReplace用の2つ目の子HWNDと、既存`core::ReplaceAllCommand`/`search::expandReplacementTemplate`への配線を追加するだけでよい(§3.19参照、`FindBar`自体の作り直しは不要)
2. **保留中のPhase 4b8**(§3.15末尾参照)に戻る選択肢もあわせて次セッション開始時にユーザーへ提示すること
3. 対話的な1行単位編集が実現したら、[ADR-011](../decisions/ADR-011-phase3c-render-cache-scope.md) の再評価トリガーに従い細粒度 DamageTracker の要否を判断する (Phase 4b1〜5b3a では未判断のまま)
4. `docs/issues/undo_stack_unbounded_memory.md` — Phase 4b1 で約1,350件規模の初回実測を追記済みだが、100万件規模の実測はまだ無い。編集量が増える機能が加わったら再実測を検討
5. `docs/issues/replace_all_buffer_snapshot_extract_scaling.md` — Phase 5b3bでReplace UIから大量マッチの置換が可能になった時点で、`replace_all_bench.cpp`の実測を検討
6. `docs/issues/match_highlight_linear_scan_scaling.md` — マッチハイライトの線形走査コスト、大量マッチ経路(Phase 5c Grep等)ができてから再評価
7. RE2/Abseilの`MSVC_RUNTIME_LIBRARY`強制上書き(`cmake/Dependencies.cmake`の`neomifes_collect_targets_recursive()`)は、Abseil/RE2のバージョンを更新する際に同じロジックが引き続き妥当か再確認すること(Abseilの`ABSL_MSVC_STATIC_RUNTIME`オプションの挙動が変わっていないか)
