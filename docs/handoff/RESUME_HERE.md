...
> **最終更新:** 2026-07-17 (Phase 4b6d 完了後)
...
| **Phase 4b7 (未スコープ)** | ⏭️ **次回スコープ確認から着手** |
...
- [x] Ctrl+矢印で単語境界へ移動する(現在行内、ユニットテスト+実アプリ確認)
- [x] Ctrl+C/X/Vでプライマリカーソルの選択範囲をコピー/切り取り/貼り付けできる(実クリップボードのラウンドトリップテスト+実アプリ確認)
...
- [ ] **既知の制限:** `RenderPipeline`はキャレット位置・選択範囲を1個(プライマリカーソルの分)しか保持しないため、Alt+クリックで追加した非プライマリカーソルのキャレット/選択ハイライトは描画されない(Phase 4b5a以降ずっと存在する制限、4b6d固有ではない)。`SelectionModel`レベルの正しさは検証済みだが、視覚的な多カーソル描画は今回のスコープ外
- [ ] 単語移動・PageUp/PageDown・選択拡張の視覚的な正しさは自動検証していない — Phase 4b1〜4b5bと同じ理由([[reference-no-win32-gui-automation]])、ユーザー自身による目視確認を推奨

**スコープ外 (Phase 4b7 以降へ持ち越し):** 複数行にまたがる単語移動、複数カーソルを跨いだクリップボードコピー/ペーストの分配、複数カーソルの視覚的描画(RenderPipeline側の大きめの変更)、ダブルクリック→ドラッグでの単語単位ドラッグ拡張、`WM_CAPTURECHANGED`の明示的ハンドリング、ドラッグ中のウィンドウ端オートスクロール、段落単位移動
...
Phase 4b7 のスコープをユーザーと確認した上で着手せよ。
着手前に detailed_design.md §5.3 の Phase 4b1〜4b6d 実装済み範囲と、
本ファイル §3.14 末尾のスコープ外一覧 (Phase 4b7候補) を確認すること。
```
...
**Phase 4b6a〜4b6d (PageUp/PageDown・Ctrl+矢印単語移動・クリップボードコピー・Alt+Shift拡張) は完了済み (§3.14 参照)。** `MovementKind`に`PageUp`/`PageDown`/`WordLeft`/`WordRight`を追加(既存の列保持ロジック・`classify()`を一般化・再利用)。新規`src/platform/clipboard.h/.cpp`でCtrl+C/X/V(プライマリカーソルの選択範囲のみ)を実装。新規`SelectionModel::moveCursorMatching()`でAlt+Shift+クリック/Alt+ドラッグによる特定カーソルの選択拡張を実装。**既知の制限:** `RenderPipeline`は依然プライマリカーソルのキャレット/選択のみ描画するため、追加カーソルの視覚的フィードバックは無い(Phase 4b5a以降の既存制限)。

**Phase 4b7 (次回) 着手時に確認すること:**
1. Phase 4b7 のスコープ自体が未確定 — §3.14 末尾のスコープ外一覧(複数行にまたがる単語移動、複数カーソルを跨いだクリップボード、複数カーソルの視覚的描画、単語単位ドラッグ拡張、`WM_CAPTURECHANGED`、ウィンドウ端オートスクロール、段落単位移動)から次に着手する範囲をユーザーに確認すること。**複数カーソルの視覚的描画**(RenderPipelineが複数のキャレット/選択範囲を保持・描画できるようにする)は、Phase 4b5a以降ずっと積み残されている既知の制限で、優先度を検討する価値がある
2. `docs/design/detailed_design.md` §5.3 に Phase 4b1〜4b6d の実装内容とスコープ外一覧を明記済み — まずここを読む
3. 対話的な1行単位編集が実現したら、[ADR-011](../decisions/ADR-011-phase3c-render-cache-scope.md) の再評価トリガーに従い細粒度 DamageTracker の要否を判断する (Phase 4b1〜4b6d では未判断のまま)
