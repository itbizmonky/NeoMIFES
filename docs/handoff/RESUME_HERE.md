...
> **最終更新:** 2026-07-21 (Phase 5b3a・5b3b・5b3c・4b8全体(4b8a〜4b8g)・5c1〜5c4・6a はpush済み、6b1・6c1・6c2 完了・未push)
...
| **Phase 6b2 (ISO-2022-JP) または Phase 6d (Document/OriginalBuffer統合) 着手 — Phase 5c5等は Phase 6完了までは候補から除外** | ⏭️ **次回** |
...
Phase 6b1(Shift-JIS/EUC-JPコーデック)・Phase 6c1(自動判定)・Phase 6c2(行末コード判定)は
...
§3.26/§3.27参照)。Phase 6a/6b1/6c1/6c2はヘッドレス実装(UI/Document結合なし)のため
視覚確認対象は無い。**
**Phase 5c5(検索履歴永続化)は Phase 6優先という本セッション内の承認済み方針により、
Phase 6完了までは次フェーズ候補として提示しないこと(§3.31参照 — 過去に不要な選択肢
提示として指摘された)。** その後、次はPhase 6b2(ISO-2022-JP — Win32のISO-2022系コードページが
厳格な入力検証を一切サポートしないことが実機検証済み、§3.30参照。正確性トレードオフへの
対応方針をユーザーに確認してから着手すること)またはPhase 6d(Document/OriginalBuffer統合、
過去に「独立した大きなサブフェーズになる見込み」と繰り返し記録されている本格着手)のどちらを
優先するかユーザーに確認してから Plan Mode で詳細設計を起こすこと(優先順位はまだ未確認)。
着手前に本ファイル §3.19/§3.20/§3.21/§3.22/§3.23/§3.24/§3.25/§3.26/§3.27/§3.28/§3.29/§3.30/§3.31
...
(§3.19/§3.20/§3.21/§3.22/§3.23参照。5c1・5c2・6a・6b1・6c1・6c2はヘッドレスのため視覚確認対象なし)。
```
