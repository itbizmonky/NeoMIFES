...
Phase 3b (DirectWrite テキストレイアウト + Document 内容の実描画 + ビューポート/スクロール位置管理) に着手せよ。
着手前に §3.5 の Phase 3a 完了記録と detailed_design.md §4 を確認すること。
```
...
| **3b** | DirectWrite テキストレイアウト、Document内容の実描画、ビューポート/スクロール位置管理 | ⏭️ **次回着手** |
| 3c | TextLayoutCache/GlyphCache/DamageTracker、60fps計測ハーネス (`--measure-frame` + CI soft-fail step) | 未着手 |
| 3d (検討) | Line Gutter、ダーク/ライトテーマ、日本語フォントフォールバック、IME | 3a〜3c完了後にスコープ確認 |

Phase 3b 着手時に確認すること:
1. `src/render/include/neomifes/render/render_pipeline.h` の現状 (`attach`/`resize`/`render` のみ、`RenderFrame`/`invalidate(TextRange)` は未実装) を起点にする
