...
> **最終更新:** 2026-07-19 (Phase 5b2 完了、push待ち)
...
| Phase 5b2 (置換 `core::ReplaceAllCommand` + `search::expandReplacementTemplate`) | ✅ 完了 (未push) |
| **Phase 5b3 (Find bar UI + コマンドパレット配線、未詳細設計)** | ⏭️ **次回着手** |
...
master_roadmap.md §5 (Phase 5b3 — Find bar UI + コマンドパレット + マッチハイライト) の
詳細設計を Plan Mode で起こしてから実装に着手せよ
(Phase 4b8 の残タスクに戻る可能性も含めて確認すること — master_roadmap.md §3 参照)。
着手前に:
1. master_roadmap.md §5 全文 (WC_EDIT子コントロール方式・FindBarState・コマンドパレット設計)
2. detailed_design.md §7.1'''(Phase 5b2実装済みのReplaceAllCommand/expandReplacementTemplate) —
   Find bar UIから実際に search:: を core:: へ繋ぐグルーコードをここで初めて書くことになる
3. 本ファイル §3.18 末尾のスコープ外一覧
これら3点を読むこと。Phase 5b2完了時点でcore::とsearch::を疎結合に保つ判断をしているため、
Phase 5b3でこの結合を実際に書く際は既存の判断根拠(§3.18参照)を踏まえること。
```
...
**Phase 5b2 (置換 core::ReplaceAllCommand + search::expandReplacementTemplate) は完了済み (§3.18 参照、未push)。** master_roadmap.md §4.3のスケッチから意図的に乖離し(core::とsearch::を疎結合に維持)、新規`core::ReplaceAllCommand`/`core::cumulative_shift_edit.h`/`search::expandReplacementTemplate`/`search::Match.groups`を実装。テスト300件(279→300)全green。

**Phase 5b3 (次回) 着手時に確認すること:**
1. Find bar UI + コマンドパレット配線の詳細設計から着手 — `master_roadmap.md` §5に既に詳細規定済み(WC_EDIT子コントロール、FindBarState、コマンドパレットのファジー検索設計まで含む)
2. `src/ui/`に既存のダイアログ/子コントロール前例が無いため、着手時に改めてExplore調査してから詳細設計すること
3. **ここで初めて`search::`が実アプリ本体(`NeoMIFES.exe`)へリンクされる。** Phase 5b2で`core::`と`search::`を疎結合に保った判断(§3.18参照)により、この時点で初めて両者を繋ぐグルーコード(`search::Match` → `core::PerCursorEdit`変換)を書くことになる — `search::expandReplacementTemplate()`と`core::ReplaceAllCommand`は既に実装済みなのでUI側の配線に専念できる
4. **保留中のPhase 4b8**(§3.15末尾参照)に戻る選択肢もあわせて次セッション開始時にユーザーへ提示すること
5. 対話的な1行単位編集が実現したら、[ADR-011](../decisions/ADR-011-phase3c-render-cache-scope.md) の再評価トリガーに従い細粒度 DamageTracker の要否を判断する (Phase 4b1〜5b2 では未判断のまま)
6. `docs/issues/undo_stack_unbounded_memory.md` — Phase 4b1 で約1,350件規模の初回実測を追記済みだが、100万件規模の実測はまだ無い。編集量が増える機能が加わったら再実測を検討
7. `docs/issues/replace_all_buffer_snapshot_extract_scaling.md` — Phase 5b3でFind bar UIから大量マッチの置換が可能になった時点で、`replace_all_bench.cpp`の実測を検討
8. RE2/Abseilの`MSVC_RUNTIME_LIBRARY`強制上書き(`cmake/Dependencies.cmake`の`neomifes_collect_targets_recursive()`)は、Abseil/RE2のバージョンを更新する際に同じロジックが引き続き妥当か再確認すること(Abseilの`ABSL_MSVC_STATIC_RUNTIME`オプションの挙動が変わっていないか)
