...
> **最終更新:** 2026-07-20 (Phase 5b3a・5b3b・5b3c・4b8全体(4b8a〜4b8g)・5c1・5c2・5c3 はpush済み、5c4・6a 完了・未push)
...
| Phase 5c4 (タグジャンプ: `parseTagJumpReference`、F12) | ✅ 完了 (未push、§3.27参照) |
| Phase 6a (Encoding Engine コア: Unicodeファミリー、ヘッドレス) | ✅ 完了 (未push、§3.28参照) |
| **Phase 6b以降 (Shift-JIS/EUC-JP/ISO-2022-JP・自動判定・Document統合) または Phase 5c5 (検索履歴永続化) 着手** | ⏭️ **次回** |
...
- [ ] Shift-JIS/EUC-JP/ISO-2022-JP・自動判定・Document統合は未着手(Phase 6b以降)
...
Phase 5c1〜5c4(GrepServiceコア・openDocumentAt・Grep結果ペインUI GrepBar・タグジャンプ)も
完了済み。5c1〜5c3はpush済み・CI success確認済み。5c4・Phase 6a(Encoding Engineコア、
Unicodeファミリー、ヘッドレス)はローカルでコミット済みだが未push —
セッション冒頭でユーザーに push 指示を仰ぐこと。
...
§3.26/§3.27参照)。Phase 6aはヘッドレス実装(UI/Document結合なし)のため視覚確認対象は無い。**
その後、次はPhase 6b(Shift-JIS/EUC-JP/ISO-2022-JPコーデック追加)またはPhase 6c(3段階自動判定、
BOM判定は6aで完成済み)に進めるか、Phase 5c5相当の検索履歴永続化(新規JSON依存のため
ADR起票が必要)を優先するか、ユーザーに確認してから Plan Mode で詳細設計を起こすこと。
着手前に本ファイル §3.19/§3.20/§3.21/§3.22/§3.23/§3.24/§3.25/§3.26/§3.27/§3.28 末尾のスコープ外一覧・
完了条件チェックボックスを読むこと。まだ実施していない実アプリでのCtrl+F/Ctrl+H/Ctrl+Shift+P/
...
§3.21/§3.22/§3.23参照。5c1・5c2・6aはヘッドレスのため視覚確認対象なし)。
```
