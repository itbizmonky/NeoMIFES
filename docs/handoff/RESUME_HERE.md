...
| **Phase 4b (キーボード/マウス入力配線・キャレット描画・Viewport↔RenderPipeline接続)** | ⏭️ **次回着手** |
...
Phase 4b (キーボード/マウス入力配線・キャレット描画・Viewport↔RenderPipeline接続) に着手せよ。
着手前に ADR-012 と detailed_design.md §5/§6 の Phase 4a 実装済み範囲を確認すること。
```
...
**Phase 4b (次回) 着手時に確認すること:**
1. [ADR-012](../decisions/ADR-012-phase4a-editor-core-scope.md) が Phase 4a で意図的に延期した項目一覧(キーボード/マウス配線・キャレット描画・矩形選択・`tryMerge`・`MovementUnit`・バケット化/圧縮・標準コマンド群・`FoldingMap`)を再評価トリガー込みで確認する
2. `MainWindow::wndProc` (`src/ui/src/main_window.cpp`) は現状 `WM_PAINT`/`WM_SIZE`/`WM_DPICHANGED`/内部初期化/`WM_ERASEBKGND`/`WM_CLOSE`/`WM_DESTROY` のみ処理。`WM_KEYDOWN`/`WM_CHAR`/`WM_LBUTTONDOWN`/`WM_MOUSEWHEEL` を新設し、`neomifes::core::CommandDispatcher`/`SelectionModel`/`Viewport` を app/ui 層で駆動する配線を設計する
3. `neomifes::core::Viewport` はヘッドレス設計 (`neomifes::render` に非依存)。`RenderPipeline::setTopLine()` (`src/render/include/neomifes/render/render_pipeline.h`) へのブリッジは app/ui 層が Viewport の `topLine()` を読んで転送する形になる想定 — 具体的な配線点は未設計
4. キャレット/選択範囲のレンダリングは `RenderPipeline` にまだ描画パスが無い。追加するかどうか・どう追加するかは Rendering Engine 側の設計判断が必要
5. 対話的な1行単位編集が実現したら、[ADR-011](../decisions/ADR-011-phase3c-render-cache-scope.md) の再評価トリガーに従い細粒度 DamageTracker の要否を判断する
6. `docs/issues/undo_stack_unbounded_memory.md` — 対話的編集が動くようになったら実メモリ計測を行う
