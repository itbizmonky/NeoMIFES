...
> **最終更新:** 2026-07-20 (Phase 5b3a・5b3b・5b3c・4b8全体(4b8a〜4b8g) はpush済み、5c1 完了・未push)
...
| Phase 4b8b (桁位置ジャンプ Ctrl+G: GotoLineBar) | ✅ 完了 (未push、§3.23参照) |
| Phase 4b8c (マーカー Ctrl+F2/F2: BookmarkManager + 最小ガター) | ✅ 完了 (未push、§3.23参照) |
| Phase 4b8d (タブ⇔スペース変換、コマンドパレット経由) | ✅ 完了 (未push、§3.23参照) |
| Phase 4b8e (フリーカーソル、簡略版) | ✅ 完了 (未push、§3.23参照) |
| Phase 4b8f (N対N分配クリップボード) | ✅ 完了 (未push、§3.23参照) |
| Phase 4b8g (キーボード矩形選択拡張 Shift+Alt+矢印 + Shift+Alt+I) | ✅ 完了 (未push、§3.23参照) |
| **Phase 4b8 全体 (4b8a〜4b8g) — roadmap上の保留項目なし、完全に完了** | ✅ **完了** |
| Phase 5c1 (GrepService コア: ヘッドレス多ファイル検索、UIなし) | ✅ 完了 (未push、§3.24参照) |
| **Phase 5c2以降 (Grep結果ペインUI等) または Phase 6 (エンコーディング) 着手** | ⏭️ **次回選択** |
...
Phase 5c1(GrepServiceコア、ヘッドレス)も完了済み。
次は以下いずれかをユーザーに選択させてから Plan Mode で詳細設計を起こすこと:
1. Phase 5c2以降 (Grep結果ペインUI・タグジャンプ・検索履歴、master_roadmap.md §5.5のスコープ外一覧参照)
2. Phase 6 (エンコーディング + 自動判定 + 10GB mmap + 遅延デコード、master_roadmap.md §6)
着手前に本ファイル §3.19/§3.20/§3.21/§3.22/§3.23/§3.24 末尾のスコープ外一覧・完了条件チェックボックスを
読むこと。まだ実施していない実アプリでのCtrl+F/Ctrl+H/Ctrl+Shift+P/Shift+Alt+ドラッグ(矩形選択)/
Ctrl+G/Ctrl+F2・F2/コマンドパレットのタブ変換2種/Toggle Free Cursor Mode/N対N貼り付け/
Shift+Alt+矢印・Shift+Alt+I/日本語IME視覚確認があれば、このセッションの冒頭でユーザーに依頼すること
(§3.19/§3.20/§3.21/§3.22/§3.23参照。5c1はヘッドレスのため視覚確認対象なし)。
```
