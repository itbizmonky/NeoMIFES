...
> # 🧊 プロジェクト凍結中 (2026-09-11) — 実装着手前に必ずここを読め

> **ユーザーの判断により開発を凍結した。完全終了ではなく、条件が整えば再開する一時停止。** フル経緯・凍結理由・再開条件・2ヶ月間(400コミット)の到達点の要約は [`docs/phase_reports/project_freeze_2026-09-11.md`](../phase_reports/project_freeze_2026-09-11.md) に集約した。**新しい実装作業(WI着手・機能追加・issue対応)は、ユーザーから明示的な凍結解除・再開の指示が無い限り自発的に始めないこと。** 技術的な質問への回答やコード調査は通常通り問題ない。

> **凍結理由の1行要約:** AI親和という差別化構想(Phase 9)はCopilot/Cursor等によるコモディティ化で個人開発が勝負できる領域ではないとユーザーが判断。AI抜きで秀丸/MIFESと同じ土俵(通常テキストエディタとしての完成度)で比較すると、現時点で明確な優位性が無い(基本操作の信頼性がまだ追いついていない — 凍結直前のWI-44/45/46で発見された3件のスクロール系バグが象徴的)。

> 以下は凍結前(2026-07-15〜2026-09-11)の全セッションの経緯記録。歴史的記録として保持する。
...
> # 🎉 最重要 (2026-09-11) — WI-44/45/46完了: ユーザー報告のスクロール系バグ3件を解消

> **ユーザーから1メッセージで3件の実機バグ報告があった:** 「1行目に入力後Enterで2行目へ移動すると1行目が見えなくなる」「IME入力がウィンドウ幅を超えても水平スクロールが追従しない」「マウスホイールで横スクロールしても左右端まで届かない」。Explore agentへ3件まとめて調査を委任し、それぞれ独立したWIとして対応した。

> **WI-44(垂直スクロール過剰反応):** `RenderPipeline::visibleLineCount()`(文書行数で頭打ちになる「今何行描画中か」)がWI-36以来`Viewport::setVisibleLineCount()`(窓の「表示容量」であるべき値)へ誤配線されていたと判明。新規`visibleRowCapacity()`(文書行数非依存の真の窓容量)へ切替。実機で1行目が表示され続けることを確認。

> **WI-45(IME水平スクロール未追従、2段階):** 1段階目、合成中はDocumentカーソルが固定されたまま`text`だけ伸びるため`ensureVisible()`の駆動経路が皆無と判明、新規`Viewport::ensureColumnVisible()`で対応。**🔴 ここでユーザーから追加報告:** 「右端の文字が右ペイン(ミニマップ)に隠れてまだ見えない」。`text.size()`(UTF-16コード単位=半角前提)が全角文字の実描画幅(DirectWriteで半角の約2倍)を過小評価していたと判明、`drawImeCompositionOnLine()`と同じDirectWrite実測パターンを流用した新規`RenderPipeline::measureTextColumnWidth()`へ差し替えて解決。実IME合成での対話的確認は`ImmGetContext()`が対象ウィンドウの`HIMC`を取得できない環境固有の制約により未完走、統合テストで代替(正直に記録)。

> **WI-46(マウスホイール水平スクロール、新規実装):** `WM_MOUSEHWHEEL`のハンドリングが一切存在しない未実装機能だったと判明。垂直方向の既存構造を鏡像として新設、実機で右端超過時のブランク表示(到達の証明)・左端0への正確な復帰の両方を確認。

> 3件とも新規issue起票の上、同一セッション内で解決済みへ。新規テスト計10件(単体7件+統合3件)追加。Debug全1621/1621件green。Release/ASan/UBSan+clang-tidyはサブエージェントへ検証委任(結果は`docs/design/build_plan.md`のWI-44/45/46セクション参照)。

> **次回セッション最初にやること:** サブエージェントの検証結果(Release/ASan/UBSan/clang-tidy)を確認し、問題なければコミット・push・CI確認。2026-08-23合意スコープ内の「今すぐ着手可能」なP1/P2 issueは(ユーザー側アクション待ちの2件を除き)尽きた状態が続いている——次にどれへ着手するかは、凍結スコープの再開も含めユーザーからの新しい方向付けを確認すること。

> 詳細は`docs/design/build_plan.md`のWI-44/45/46セクション、`docs/history/TIMELINE.md`最新セッション参照。

> ---

> # 🎉 最重要 (2026-09-11、historical) — WI-43完了: `asan`プリセットのCI常設化
...
> **(2026-09-11追記) 本コールアウトの「次回やること」はWI-44/45/46着手により上の新しいコールアウトへ差し替え済み。以下は歴史的記録として保持。**
...
> **🔴 実装前の再確認で、選択肢1の前提自体が既に成立していないと判明した。** `onDeferredInit`(計測ループの実行場所)は`main_window.cpp`の`handlePaint()`により**既に**最初の`WM_PAINT`完了後にしか発火しない設計だった——「ペイント完了を待つ」は実装済みで、これをそのまま足しても効果が無い。この食い違いを正直にユーザーへ報告した上で実装を続けた。

> **🔴 いったん`onDeferredInit`内で`IsWindowVisible()`をチェックする形で実装・ビルドしたところ、新規回帰テストが15秒タイムアウトで強制終了し、修正が効いていないことが実測で判明した。** マーカーファイル方式の一時診断コードで実機再現したところ、**非表示ウィンドウでは`onDeferredInit`自体が一切発火しない**(5秒待ってもマーカーファイル未作成)ことを実証した。

> **真因の再確定:** 一度も表示されないウィンドウは`WM_PAINT`を一切受け取らないため、`onFirstPaint`/`onDeferredInit`いずれも永久に発火せず、計測ロジック(`Present1()`含む)は一度も実行されないまま`GetMessageW()`が処理すべきメッセージを一切受け取れず無期限ブロックする——**WI-41が疑ったPresent1のvsync待ちではなく、より単純な「メッセージキューが空のまま誰も`requestClose()`を呼ばない」ハングだった。** 再度ユーザーへ報告し、非表示検知後の挙動を確認、「エラーで即座に終了」が選ばれた。

> **修正:** `wWinMain()`の`window.create()`が返った直後(`MeasureFrame`限定)へ`IsWindowVisible(window.hwnd())`のチェックを追加。`create()`内の`ShowWindow()`/`UpdateWindow()`は同期呼び出しのため`create()`が返った時点で`WS_VISIBLE`は確定しており、「まだペイントされていない」と「今後も一切ペイントされない」を区別できる最も早いタイミングだった。非表示検知時は終了コード3で即座に終了(プロファイルJSONは書かない)。スコープはユーザー承認のもと`--measure-frame`のみに限定、`MeasureStartup`/`MeasureMemory`の同型脆弱性の可能性は未検証のまま対象外(推測実装回避)。

> 新規回帰テスト`FrameMeasureTest.HiddenWindowFailsFastInsteadOfHanging`(`tests/integration/frame_measure_test.cpp`)を追加。`STARTF_USESHOWWINDOW`+`SW_HIDE`での実起動を再現し、修正前は15秒タイムアウト強制終了・修正後は約40msで完了することを実測確認。

> Debug全1614/1614件green(既存回帰無し)。Release/ASan/UBSan(clang-cl)3構成もサブエージェントへ委任し検証済み(詳細は`docs/design/build_plan.md`のWI-42セクション参照)。
...
> **(2026-09-11追記) 本コールアウトの「次回やること」はWI-41調査・WI-42着手により上の新しいコールアウトへ差し替え済み。以下は歴史的記録として保持。**
...
> # 🎉 最重要 (2026-09-07) — WI-35完了: 「1行目でガタつく」の真因を特定・解消(`SIF_DISABLENOSCROLL`)

> **WI-34完了後、ユーザーが実機で切り分けた結果を報告:「事象は水平スクロールバーをOFFにしたら発生しなくなった。水平・垂直スクロールバーはONの場合は常に表示する。文字数により表示非表示としない。」** WI-31〜WI-34を通じて技術的原因を特定できなかった「1行目のみガタつく」謎に、**ユーザー自身の実機切り分けで初めて再現性のある手がかりが得られた。**

> **真因:** `syncHorizontalScrollBar()`/`syncVerticalScrollBar()`が`SetScrollInfo()`に`SIF_DISABLENOSCROLL`を指定していなかったため、Win32標準スクロールバーが範囲(`nMax`=行の文字数近似)がページサイズ(`nPage`=可視列数)に収まると判断した瞬間、**暗黙に自動非表示になっていた。** 1行目に文字を打つ過程でこの大小関係が頻繁に入れ替わり、その度にネイティブスクロールバーが消えたり現れたりしてクライアント領域の高さが変化——これがレイアウトのズレの直接的な実体だった。**この機構は`RenderPipeline::resize()`を一切経由しないため、WI-34の`resize()`計装では原理的に検出不可能だった**(計装対象が誤っていたのではなく、計装が届かない層で起きていた)。

> **実測検証:** 修正前ビルドで新規ファイルの1行目へ250文字連続入力しクライアント高さを記録したところ、**741px⇔724pxの間で11回の切り替わりを実測**——これが「ガタつき」の直接的な実体。`SCROLLINFO::fMask`へ`SIF_DISABLENOSCROLL`を追加する1行の修正で、**修正後は同一実験で遷移0回**(完全に一定)に収束することを`git stash`での修正前後ビルド比較で確認した。

> Debug全1614/1614件green、clang-tidy新規指摘0件。**Release/ASan/UBSan(clang-cl)3構成もサブエージェントへ委任し全構成1614/1614件green、警告0件、サニタイザ診断0件を確認済み。** 検証中、3構成を並列実行するとリソース競合でASanのctestが無応答になる事象が発生したため、ASanのみ単独再実行して完走を確認した(今後の運用上の注意点として記録)。コミット`3f68ed8`。

> **(2026-09-08追記) 本コールアウトの「次回やること」はその後の展開(push実施+CI障害の即日修正)により上の新しいコールアウトへ差し替え済み。以下は歴史的記録として保持。**

> 詳細は`docs/design/build_plan.md`のWI-35セクション、`docs/history/TIMELINE.md`最新セッション参照。

> ---

> # 🎉 最重要 (2026-09-06) — WI-34完了: スクロールバー表示設定の追加 + 本物の縦スクロールバーの新規実装

> **WI-33完了後、ユーザーから新規報告:「２行目移行の文字入力ではウィンドウの再描写は制限されているが、１行目の文字入力では以前として頻繁に再描写される。」** 実測ドッグフーディング(定常タイピング、真のOS `keybd_event`キー押しっぱなし、ウィンドウ座標計測)を多角的に試みたが、**「1行目固有」のコードパスは発見できなかった。** 調査序盤に得た「1行目だけ多い」ように見える結果は、起動とキー入力送信を別ツール呼び出しに分けたことによる測定アーティファクトと自ら気づき訂正した(このセッションの教訓として`reference_no_win32_gui_automation.md`系メモリに追記推奨)。

> **ユーザーの仮説と機能要望:** 「下部のステータスバーの横スクロールバーが文字入力中のみ表示されるため画面全体のレイアウトがズレてチラついているように見える。縦横のスクロールバーは設定画面からON/OFFにできるようにして自動で表示・非表示を制御しないように修正して欲しい」との提案。`RenderPipeline::resize()`呼び出し回数を計測してこの仮説を検証したが、**タイピング中に`resize()`は一切呼ばれておらず、仮説の直接的な技術的裏付けは得られなかった。** この結果を正直に報告した上で、ユーザーは「原因は不明なままだが、提案の設定機能を実装する」と続行を決定した。

> **実装:** 横スクロールバーの表示設定化に加え、**NeoMIFESに一度も存在しなかった本物の縦スクロールバーを新規実装**(`WS_VSCROLL`/`WM_VSCROLL`/`computeVScrollTargetLine()`/`syncVerticalScrollBar()`、WI-03の横スクロールバー実装の完全な鏡像として設計)。設定でのON/OFF・メニュー/コマンドパレットのトグルは既存の流儀通り。Plan agentの設計内容に1件の誤り(`RenderPipeline::visibleLineRange()`が「既にpublic」との記載、実際はprivate)を発見・修正し新規publicラッパー`visibleLineCount()`を追加した。
...
> # 🎉 最重要 (2026-09-06) — WI-33完了: 「1行目入力時に依然としてガタつく」を修正(非同期シンタックストークンの二重描画)

> **WI-32完了報告後、ユーザーから新規報告:「テキスト１行目入力時には依然として再描写が頻繁に発生して画面がカクつく。秀丸の快適さを超えるユーザビリティを実現せよ。」** 2件のExplore agentへ調査を委任し、①WI-32の入力キューのドレイン優先は真の滞留がある場合のみ効き通常速度のタイピングでは1キー入力=最低1vblankの同期Presentが残る(構造的制約、1行目固有ではない)、②`handleAppMessage()`の`kMsgSyntaxTokensReady`分岐がWI-32のゲートを経由しない生`InvalidateRect`を呼んでおり、文書が短い(新規ファイル・1行目付近)ほど非同期シンタックス解析が高速完了してほぼ毎キー入力ごとに2回目の独立した再描画を起こしていた、の2点を特定した。

> **修正(②、確定原因):** `kMsgSyntaxTokensReady`ハンドラへ新規`requestPaintForBackgroundUpdate()`(`RenderPipeline::paintOverdue(50ms)`のみで判定)を適用。**設計上の教訓:** 当初WI-32の`requestPaintUnlessBusy()`(キュー状態で判定)をそのまま再利用する設計で実装したが、実測ドッグフーディングで「通常速度のタイピングでは非同期トークン到着時もキューが空のため修正が効かない」ことを発見し、実装完了前に設計を修正した——既存ヘルパーを安易に再利用せず実測で検証したことが功を奏した実例。

> **実測:** 新規`.cpp`ファイルへ`"#include <iostream>"`(20文字)を70ms間隔でタイプ入力し、実際の`renderOnce()`呼び出し回数が**38回(修正前、≈2回/キー入力)→20回(修正後、1回/キー入力)**に収束することを確認。スクリーンショットでシンタックスハイライトも正しく反映されることを確認。

> Debug/Release/ASan/UBSan(clang-cl)全1602/1602件green、clang-tidy新規指摘0件。コミット`f9b5be3`。

> **残課題(①、本WIのスコープ外):** メッセージキューに滞留が無い通常速度のタイピングでも1キー入力ごとに最低1vblankの同期Present待ちが発生するという、より根本的な制約は未解決のまま[`keystroke_burst_render_backlog.md`](../issues/keystroke_burst_render_backlog.md)に記録。解消にはSyncInterval=0への変更や描画スレッド分離等、ADR-011を覆すレベルの大規模変更を要する。**ユーザーから「秀丸の快適さを超えるユーザビリティを実現せよ」との要望が出ているため、この残課題への対応可否を次回セッションでユーザーへ改めて確認すること。**
...
> **結論: CPU側の描画コストは常に3ms未満(問題なし)。原因は「1キー入力ごとに同期的にvsync待ちPresent(`Present1(1,0,...)`)を行う」というメッセージループ/描画パイプラインのアーキテクチャそのもの。** 現実的なタイピング間隔(130ms/文字)では無症状だが、バースト間隔(20ms/文字、単語内の連続打鍵等で普通に起きる速度)を模擬注入すると、フレーム開始間隔が注入間隔ではなく一貫して30〜32ms(60Hzの1〜2vblank分)に張り付き、WM_CHARがメッセージキューに滞留して画面表示が入力に追いつかず後追いになることを実測で確認した——これが「ガタつく」の実体。詳細・実測数値は[`docs/issues/keystroke_burst_render_backlog.md`](../issues/keystroke_burst_render_backlog.md)(新規、P1)参照。
