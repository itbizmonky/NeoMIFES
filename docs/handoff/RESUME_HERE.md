...
> **最終更新:** 2026-07-17 (Phase 4b7c 完了後)
...
| **Phase 4b8 (未スコープ)** | ⏭️ **次回スコープ確認から着手** |
...
Phase 4b8 のスコープをユーザーと確認した上で着手せよ。
着手前に detailed_design.md §5.3 の Phase 4b1〜4b7c 実装済み範囲と、
本ファイル §3.15 末尾のスコープ外一覧 (Phase 4b8候補) を確認すること。
```
...
**Phase 4b8 (次回) 着手時に確認すること:**
1. Phase 4b8 のスコープ自体が未確定 — §3.15 末尾のスコープ外一覧(複数カーソルを跨いだクリップボードのN対N分配、段落単位移動、単語単位ドラッグ拡張、`WM_CAPTURECHANGED`、ウィンドウ端オートスクロール)から次に着手する範囲をユーザーに確認すること
2. `docs/design/detailed_design.md` §5.3 に Phase 4b1〜4b7c の実装内容とスコープ外一覧を明記済み — まずここを読む
3. 対話的な1行単位編集が実現したら、[ADR-011](../decisions/ADR-011-phase3c-render-cache-scope.md) の再評価トリガーに従い細粒度 DamageTracker の要否を判断する (Phase 4b1〜4b7c では未判断のまま)
