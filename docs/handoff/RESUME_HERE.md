...
> **最終更新:** 2026-07-19 (Phase 5b3a・5b3b 完了、push待ち)
...
| **Phase 5b3c (コマンドパレット Ctrl+Shift+P)** | ⏭️ **次回着手** |
...
Phase 5b3c (コマンドパレット Ctrl+Shift+P) の詳細設計を Plan Mode で起こしてから実装に着手せよ
(Phase 4b8 の残タスクに戻る可能性も含めて確認すること — master_roadmap.md §3 参照)。
着手前に:
1. detailed_design.md §7.1''''/§7.1'''''(Phase 5b3a/5b3b実装済みのui::FindBar/FindReplaceState) —
   コマンドパレットは完全に独立したUI表面だが、5b3aで確立したWC_EDIT+サブクラス化パターンを
   再利用しつつ新規クラスとして設計すること(FindBarへの機能追加ではない)
2. master_roadmap.md §5.2後半(コマンドパレットの元スケッチ)
3. 本ファイル §3.19/§3.20 末尾のスコープ外一覧
これら3点を読むこと。まだ実施していない実アプリでのCtrl+F/Ctrl+H/日本語IME視覚確認があれば
このセッションの冒頭でユーザーに依頼すること(§3.19/§3.20参照)。
```
