...
> **次にやること: 🎉 M1 ドッグフーディング再確認 (ユーザー実施) → WI-03 (横スクロール)** — `build_plan.md` §5 参照。WI-01 (文書保存基盤) は完了・コミット済み (`a4a0445`)。WI-02 (ファイルライフサイクル UI) は実装・コミット済み (`3e611d8`)。**ユーザーが実際にドッグフーディングを試み、2件の実害あるバグ (Ctrl+O後の画面未反映/マウスホイールEOF超過スクロール) を発見・報告した。両方とも本セッション中に根本原因を特定・修正・回帰テストで実証済み (§3.69参照)、コミット待ち。** ユーザーへ再度ドッグフーディングの実施を依頼中 — 今度こそ問題なく編集・保存・コミットできるか確認後、WI-03 (横スクロール) へ進む。
>
> ---
...
| **8.5b** | **ファイルライフサイクル UI** (Ctrl+S/O/N、`IFileDialog`、D&D、未保存警告) | 🟡 **実装完了 (WI-02、コミット済み`3e611d8`、§3.68参照)。ドッグフーディングで2件のバグ発覚→修正済み (§3.69参照)、pushはユーザー指示待ち、🎉 M1は再確認待ち** |
...
- [ ] 🎉 **ユーザーによる再ドッグフーディング確認** ← 未実施、依頼中

**コミット済み`5712435`、pushはユーザーの明示指示待ち。** ユーザーによる再確認後、🎉 M1達成を正式に記録し、次はWI-03(横スクロール)。
...
で実証済み (Debug/Release/ubsan全1002/1002 green)、コミット待ちの状態。

ユーザーへ再度ドッグフーディングの実施を依頼し、確認が得られたら、
build_plan.md §3 の WI-02 行にコミットハッシュを記入し M1 達成を正式に
記録した上で、WI-03 (横スクロール、build_plan.md §5 参照) に着手すること。
