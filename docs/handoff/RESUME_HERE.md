...
> **最終更新:** 2026-07-19 (Phase 5b3a 完了、push待ち)
...
| **Phase 5b3b (置換行配線 Ctrl+H)** | ⏭️ **次回着手** |
| Phase 5b3c (コマンドパレット Ctrl+Shift+P) | ⏸️ 5b3b完了後に着手 |
...
Phase 5b3b (置換行配線 Ctrl+H) の詳細設計を Plan Mode で起こしてから実装に着手せよ
(Phase 4b8 の残タスクに戻る可能性も含めて確認すること — master_roadmap.md §3 参照)。
着手前に:
1. detailed_design.md §7.1''''(Phase 5b3a実装済みのui::FindBar/MatchVisual) —
   currentQuery/currentMatches/currentMatchIndexは main.cpp 側にローカル状態として
   既に存在しているため、5b3bはReplace用の2つ目の子HWNDと、既存の
   core::ReplaceAllCommand/search::expandReplacementTemplateへの配線を追加するだけでよい
2. detailed_design.md §7.1'''(Phase 5b2実装済みのReplaceAllCommand/expandReplacementTemplate)
3. 本ファイル §3.19 末尾のスコープ外一覧
これら3点を読むこと。まだ実施していない実アプリでのCtrl+F/日本語IME視覚確認があれば
このセッションの冒頭でユーザーに依頼すること(§3.19参照)。
```
