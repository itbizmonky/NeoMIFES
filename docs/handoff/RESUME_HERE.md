...
Phase 5b2 (置換 ReplaceAllCommand) の設計から着手せよ
(Phase 4b8 の残タスクに戻る可能性も含めて確認すること)。
着手前に detailed_design.md §7 の Phase 5b1 実装済み範囲(§7.1')と、
本ファイル §3.17 末尾のスコープ外一覧(ReplaceAllCommandの設計上の
制約 — 既存MultiCursorEditCommandは転用不可、core::ICommand直接実装が
必要と判明済み)を確認すること。
```
