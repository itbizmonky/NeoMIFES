...
AskUserQuestionで今後の扱いを確認したところ、用意した3択(そのままアイドルで完走/能動的ソークに作り直して再スタート/両方実施)のいずれでもなく、**「12時間としてアイドル確認で完走させる」というカスタム回答**が選ばれた(既に約10時間分の待機投資を無駄にせず、かつ24時間という誤解を招く期間表示を修正する折衷案)。`soak_monitor.ps1`の閾値を24→12へ変更(完了予定 2026-08-31 23:04頃)、`master_roadmap.md`§12.5の「100万Undo」項目と「クラッシュ0」項目を統合・再定義(能力達成/実使用ソーク未実施を明記)、新規issue [`undo_redo_active_usage_soak_not_performed.md`](../issues/undo_redo_active_usage_soak_not_performed.md)(P2)を起票、`docs/issues/README.md`/`RESUME_HERE.md`へ反映。ドキュメント同期のみでコミットは未実施(次回セッション、ソーク完了確認と合わせてまとめてコミットする想定)。
