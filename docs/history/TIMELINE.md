...
**続けて、ユーザーから「起動放置して24時間経過観察しても意味がないのでは」と鋭い指摘を受けた。** `soak_monitor.ps1`の中身を確認し、指摘が正しいことを確認: 15分おきにプロセス生存とWorkingSetを記録するだけで、実際の入力・編集・Undo/Redo等は一切行わないアイドル放置確認だった。さらに調査すると、`master_roadmap.md`§12.3原案は「100万回連続Undo/Redoを24時間」という能動的ストレステストを計画しており、チェックリスト項目名「100万Undo(24時間ソーク)」の中身が計画時点から別物(アイドル確認)に置き換わっていたという構造的な不整合も判明した(ただし100万Undo自体の能力・速度は既存ベンチマークで実測済みのため機能欠陥ではない)。

AskUserQuestionで今後の扱いを確認したところ、用意した3択(そのままアイドルで完走/能動的ソークに作り直して再スタート/両方実施)のいずれでもなく、**「12時間としてアイドル確認で完走させる」というカスタム回答**が選ばれた(既に約10時間分の待機投資を無駄にせず、かつ24時間という誤解を招く期間表示を修正する折衷案)。`soak_monitor.ps1`の閾値を24→12へ変更(完了予定 2026-08-31 23:04頃)、`master_roadmap.md`§12.5の「100万Undo」項目と「クラッシュ0」項目を統合・再定義(能力達成/実使用ソーク未実施を明記)、新規issue [`undo_redo_active_usage_soak_not_performed.md`](../issues/undo_redo_active_usage_soak_not_performed.md)(P2)を起票、`docs/issues/README.md`/`RESUME_HERE.md`へ反映。コミット(`e7db805`)・push完了。

**日付が変わり2026-09-01になったセッション冒頭、ユーザーから「v1出荷判定は既に終了している認識、昨日の23:04に完了している」と指摘され、`soak_log.csv`を確認したところ2026-08-31T23:04:39に`SOAK_COMPLETE_NO_CRASH`が記録されており、その通りだった(前回セッションの筆者が「本日23:04頃」と書いた際、日付が変わっていることを見落としていた)。** タスクスケジューラは設計通り自動的に登録解除・プロセス終了済みを確認。WorkingSet推移(50.71MB起動直後→57.58MB定常5時間→95〜97MB、AppVerifier検証の一時再起動後の水準で以降横ばい、単調増加なし)を`master_roadmap.md`§12.5の「クラッシュ0/メモリ安定性」項目へ実測転記した。**これでv1出荷判定17項目中16項目達成(3項目部分達成)、残り1項目(数GB Grep)のみ未達確定 — 全項目の検証が完了した。** `RESUME_HERE.md`も合わせて更新。

次はユーザーへ、新規発見5件のいずれかへ着手するか、WI-13前例に倣い未達項目(数GB Grep)を正直に記録したまま現状でv1出荷判定(🎉M5)を宣言するかを確認する。

**AskUserQuestionで確認したところ「現状のままM5を宣言する(推奨)」が選ばれ、🎉M5(v1出荷、軽量版)を正式に達成として確定した(2026-09-01)。** WI-13(M4)前例(未達2項目を正直に記録したままユーザー承認で確定)を踏襲し、v1出荷判定17項目中16項目達成(3項目部分達成)・残り1項目(数GB Grep)は未達のまま記録、新規発見5件のissueは次フェーズ候補として保留。`build_plan.md`(§0現在のゴール+§3進捗チェックリスト、次フェーズ候補リスト新設)/`master_roadmap.md`/`RESUME_HERE.md`へ反映。ソーク検証用のスクラッチファイル(`.tmp_v1_verify\soak_monitor.ps1`/`soak_log.csv`/`soak_start.txt`)は結果転記後に削除した(WI-13前例踏襲)。
...
**続けてユーザーが「進めよ」と指示、WI-21e(折り返し実配線+表示メニュー拡充)を完了(2026-09-03)。🎉 折り返し機能が初めてユーザーから実際に触れるようになった。**

### WI-21e実装

**`core::Viewport::setWordWrapEnabled(bool)`。** `ensureVisible()`の水平クランプ処理を折り返し有効時は完全にスキップするよう変更した(設計時点で発見していた見落とし通り、水平スクロールバーを非表示にするだけではクランプ処理自体は生き続け、End/Ctrl+Right等の操作でユーザーに見えないまま`m_leftColumn`がずれる実害が起きるため)。OFF→ON遷移時のみ`m_leftColumn`を0にリセットする設計にした。

**`RenderPipeline::wordWrapEnabled()`新規publicゲッター**+**新規`CommandId`3種(`WordWrapToggle`/`LineNumbersToggle`/`ThemeCycle`)**。いずれもメニュー/パレット専用でキーボードショートカット無し(`kAllRemappableCommandIds`には含めない、`ToggleOverwriteMode`と同じ「承認済みの設計判断が無い限りキー割当は追加しない」扱い)。`ThemeCycle`は既存の`view.theme.dark/light/highContrast`3コマンドを廃止せず併存させ、Dark→Light→HighContrast→Darkを固定順で巡回する専用コマンドにした(フラットな最上位メニューには「最近使ったファイル」のようなサブメニュー機構が無いため)。`kViewMenuItems`を3→6項目へ拡張。`dispatchWidgetShowCommand()`に3ケース追加(`core::Settings&`+`settingsPath`を新規パラメータとして受け取るようシグネチャ変更)、コマンドパレット側は新規`appendViewToggleCommands()`に同一ロジックを重複実装した——`JsonTreeToggle`/`CsvGridToggle`が既に確立している「小さなトグル本体はパレット/メニューそれぞれに独立してコピーする」という明文化された慣習に倣い、共有ヘルパーへは括り出さなかった。

**`syncHorizontalScrollBar()`が`renderPipeline.wordWrapEnabled()`を見て水平スクロールバーを表示/非表示するようになった。** `handlePaintEvent()`に`session.viewport().setWordWrapEnabled(renderPipeline.wordWrapEnabled())`という毎フレーム同期を追加——折り返しは`RenderPipeline`側でウィンドウ全体1つのグローバル状態だが`core::Viewport`はセッション(タブ)ごとに存在するため、既存の`setVisibleColumnCount(...)`と全く同じ「毎フレーム最新値を反映する」設計で、タブ切替/新規タブ作成/起動直後のどのタイミングでも自動的に正しい状態へ追従させた(`syncViewForActiveSession()`の全14箇所前後の呼び出し元を個別に追跡する代わりに、この1行で完結する設計判断)。起動時配線(`session_manager.cpp`/`main.cpp`)と`settings.reload`コマンドにも`renderPipeline.setWordWrap(settings.wordWrap)`を追加した。

**実装中に発見・修正した小さな不備:** `ThemeCycle`のロジックをネストした三項演算子で書いたところ、clang-tidyの`readability-avoid-nested-conditional-operator`(このプロジェクトの`-WX`下ではエラー)に2箇所(パレット側/メニュー側の重複実装それぞれ)で検出された。`switch`文ベースの新規`nextThemeKind()`ヘルパー(`theme_settings.h`、`parseThemeKind()`/`themeKindToSettingsString()`と同じ配置)へ書き換えて解消し、重複していたロジックも1関数へ統合した(値のみを返す純粋関数のため、状態変更を伴う「トグル本体は重複させる」慣習の対象外と判断)。

### 🔴 実機ドッグフーディングで発見した重大バグ: `FrameState`に`wordWrapEnabled`が含まれていなかった

**本WIで初めて実機ドッグフーディングを実施したところ、`WordWrapToggle`をメニュー/コマンドパレットから実行しても画面に一切反映されないという重大な実害を発見した。** 長い1行を含むテスト用ファイルをワイドウィンドウで開き、`CommandId::WordWrapToggle`(WM_COMMAND直接送信、値は`command_ids.h`のenum宣言順から手計算した40009)を送信して確認したが、テキストは折り返されず横スクロールバーも消えなかった。`settings.json`を直接読むと`wordWrap:true`が正しく永続化されており、ロジック自体は実行されていると判明——しかし画面には一切反映されない。**不可解なことに、ウィンドウをリサイズすると突如として正しく折り返されるようになった。**

原因調査の結果、`RenderPipeline::FrameState`(`render()`の粗粒度フレームスキップ判定、ADR-011)に`wordWrapEnabled`が一度も含まれていなかったと判明した——**WI-15iの`rightPaneWidthDips`、WI-21bの`showMinimap`/`showLineNumbers`に続く、全く同じ問題クラスの3度目の再発。** `setWordWrap(true)`自体は`m_wordWrapEnabled`更新+レイアウトキャッシュクリアを正しく行っていたが、それ以外のFrameStateの全フィールド(文書/topLine/カーソル/サイズ等)が不変なままトグルだけが実行される場面(対話的なメニュークリックがまさにこの状況を作る)では、`render()`が「何も変わっていない」と誤判定し描画パス全体を丸ごとスキップしていた。リサイズは`width`/`height`がFrameStateに含まれるため、たまたま無関係な再描画のトリガーとして機能していただけだった。

**WI-21b〜dの自動テストが誰もこれを検出できなかった理由も特定した。** いずれのテストも`setWordWrap()`をそのパイプラインの**最初の`render()`より前**に呼んでいたため、`m_lastRenderedFrameState`がまだ`nullopt`(比較対象が無い)の状態で、フレームスキップ判定自体が発動せず常に素通りしていた。実際に発生したバグは「既に1回以上描画済みのセッションに対して、単独でトグルする」という対話的な使用パターンでしか顕在化しない——自動テストでは原理的に発見不可能で、実機ドッグフーディングでのみ発見できた実例だったと言える。

`FrameState`に`bool wordWrapEnabled = false;`フィールドを追加、`captureFrameState()`側の初期化も追加して修正(`rightPaneWidthDips`/`showMinimap`/`showLineNumbers`と全く同じ修正パターン)。新規回帰テスト`SetWordWrapAloneAfterFirstRenderIsNotCoarseFrameSkipped`(`tests/integration/render_text_smoke_test.cpp`)を追加——「最初の`render()`を先に済ませてから`setWordWrap()`単体を呼ぶ」という、まさにこのバグを再現する手順を明示的にテスト化した。

### ビルド時に遭遇した既知の環境問題

`FrameState`修正後の再ビルド中、ドッグフーディング用に起動していたNeoMIFES.exeプロセスがexeファイルをロックしたままとなり(`LNK1104`)、`Stop-Process`が「アクセスが拒否されました」で失敗する既知の「killできないゾンビNeoMIFESプロセス」に再度遭遇した。確立済みの回避策(ロックされた`NeoMIFES.exe`を別名へリネームし新規ビルドを通す)で対処、再ビルド後に正常に検証を継続できた。

### 実機ドッグフーディング(全て確認済み)

WM_COMMAND直接送信(`WordWrapToggle`=40009/`LineNumbersToggle`=40010/`ThemeCycle`=40011)+スクリーンショット+`GetWindowLong`/`settings.json`直接確認の組み合わせで以下を確認した。

- **折り返しON(`FrameState`修正後)**: ワイドウィンドウでリサイズ無しに長い行が正しく複数行へ折り返されることを確認。`GetWindowLong`で`WS_HSCROLL`スタイルビットが消えていることを確認し、水平スクロールバーが実際に非表示になっていることを裏付けた。
- **折り返しOFF(トグルバック)**: 折り返しが解除され元の1行表示に戻ること、`WS_HSCROLL`が復活し水平スクロールバーが再表示されることを確認。
- **行番号トグル**: ガターの行番号(1,2,3...)が消え、テキスト開始位置が左へシフトすることを確認。
- **テーマ切替**: Dark→Lightへ正しく切り替わる(背景色反転)ことを確認。
- **表示メニューの目視確認**: 表示(V)メニューを実際にマウスクリック(`SetCursorPos`+`mouse_event`)で開き、既存3項目(アウトライン/構造ツリー/CSVグリッド)に続き新規3項目(折り返し(W)/行番号(L)/テーマ切替(T))が正しい日本語ラベル・キーボードショートカット無しの表示で並んでいることをスクリーンショットで確認。
- **再起動後の永続化**: 折り返しOFF/行番号非表示/テーマLightの状態でプロセスを終了→再起動したところ、`settings.json`から正しく読み込まれ同じ表示状態(スクリーンショットで再確認)で起動することを確認——起動時配線(`session_manager.cpp`)の検証を兼ねる。

### 最終ゲート

Debug/Release/ubsan全1566/1566件green(3構成とも実行、`FrameState`修正+回帰テスト追加という2段階の変更を経た**最終**状態に対して再検証、レースコンディションの懸念から念のためRelease/ubsanを2回検証しどちらも green を確認)。clang-tidy exit code 0(`render_pipeline.h`/`render_pipeline.cpp`/`normal_mode_wiring.cpp`/`viewport.cpp`/`session_manager.cpp`/`main.cpp`/`theme_settings.h`とも新規警告なし、ネストした三項演算子1件検出・修正)。実機ドッグフーディング完了(上記参照)。
...
WI-28完了報告直後、ユーザーから新規報告:「まだキー入力すると全再描写されてウィンドウの描写がガタつく。」ステータスバーのチラつき(WI-28)とは別の、メインのテキスト描画エリアを含む再描画自体の体感問題。

2件のExplore agentへ調査を委任し、二重invalidateなし・フレームスキップはタイピング中実質無効(ADR-011の意図的設計)・`Present1(1,0,...)`はSyncInterval=1(vsync同期)であることを確認。`docs/history/TIMELINE.md`横断調査で、Phase 3c以降ほぼ全WIで`--measure-frame`実測値が一貫して`avgFrameNs≈16.5ms`(60Hzの1vblank間隔そのもの)であり、既存ベンチ(`BM_TextLayoutCache_Miss`=542ns/行)からCPU側描画コストが無視できる規模と分かった。「フレーム時間はCPU負荷でなくvsync待ちに支配されている」との仮説を立て、CLAUDE.mdルール10に従いタイピング固有のレイテンシを実測診断した。

`RenderPipeline::render()`へ`platform::PerfClock`ベースの一時計装(`DOGFOOD-TEMP`タグ)を追加し、Debugビルドへ確立済みハイブリッド技法で2種類の速度のタイピングを注入した。**現実的な間隔(130ms/文字)では問題なし(フレーム間隔が注入間隔とほぼ一致、renderOnce()所要時間0.5〜2.9ms)。バースト間隔(20ms/文字)では、renderOnce()自体のCPUコストは同水準のまま、フレーム開始間隔が注入した20msでなく一貫して30〜32ms(60Hzの1〜2vblank分)に張り付き、26文字の注入(約520ms)に対し25フレーム分の処理完了に約780ms要した——入力到着速度がレンダリング処理速度を上回りWM_CHARがメッセージキューに滞留、表示が入力に追いつかず後追いになる現象を実測で確認した。**
...
WI-32完了報告後、ユーザーから新規報告:「テキスト１行目入力時には依然として再描写が頻繁に発生して画面がカクつく。秀丸の快適さを超えるユーザビリティを実現せよ。」2件のExplore agentへ調査を委任した。

**①(構造的制約、1行目固有ではない):** WI-32の入力キューのドレイン優先は真の滞留がある場合のみ効く。通常速度のタイピングでは、前のキー入力の処理がPresent1のvsync待ちを含めて完了してから次のキー入力が到着するため、キューには何も残らず修正前と同じ「1キー入力=最低1vblankの同期Present」が発生する——これは文書内のどこでタイプしても同じ。

**②(1行目/新規ファイル固有、決定的原因):** `handleAppMessage()`の`kMsgSyntaxTokensReady`分岐が、WI-32のゲートを経由しない生の`InvalidateRect`を呼んでいた。`SyntaxWorker`は編集の度に文書全体を再走査するため、文書が短い(新規ファイル・1行目付近)ほどこの非同期処理が高速に完了し、編集自体の再描画とは別にほぼ毎キー入力ごとに2回目の独立した再描画が発生していた。`applyGitDiffReadyMessage()`/`applyLogIndexReadyMessage()`にも同型の生`InvalidateRect`があるが、いずれもユーザーの明示コマンドからのみ発火し毎編集の自動発火ではないため対象外と判断した。

**設計上の教訓(実装完了前に実測で発見・修正):** `kMsgSyntaxTokensReady`ハンドラの修正として、当初WI-32の`requestPaintUnlessBusy()`(キュー状態で判定)をそのまま再利用する設計で実装したが、**実測ドッグフーディングで「通常速度のタイピングでは非同期トークン到着時もキューが空のため、修正が効かず二重描画が解消されない」ことを発見**した。背景ワーカー完了による再描画は次の実描画に乗せればよく緊急性が無いのに対し、ユーザー自身のキー入力による再描画は即時性が必要——この非対称性を反映し、`paintOverdue(50ms)`のみで判定する新規`requestPaintForBackgroundUpdate()`へ設計を修正した。「既存ヘルパーを安易に再利用せず、実測で検証する」という本セッション一貫の姿勢が、実装完了前に設計ミスを捕捉した一例。

**実測:** 新規`.cpp`ファイルへ`"#include <iostream>"`(20文字)を70ms間隔でタイプ入力し、実際の`RenderPipeline::render()`呼び出し回数を計測。修正前(設計v1の誤り含む)は38回(≈2回/キー入力)、修正後(v2)は**20回(1回/キー入力)**に正確に収束することを確認。スクリーンショットでシンタックスハイライトも正しく反映されることを確認。

Debug全1602/1602件green、clang-tidy新規指摘0件。Release/ASan/UBSan(clang-cl)はサブエージェントへ検証委任。**残課題(①の構造的制約)は`keystroke_burst_render_backlog.md`に記録、ADR-011を覆す大規模変更が必要なため本WIのスコープ外。**

### WI-34: スクロールバー表示設定の追加 + 本物の縦スクロールバーの新規実装

WI-33完了後、ユーザーから新規報告:「２行目移行の文字入力ではウィンドウの再描写は制限されているが、１行目の文字入力では以前として頻繁に再描写される。これでは製品化は難しいので改善して欲しい」。

**再現調査:** 実測ドッグフーディング(定常タイピング、真のOS `keybd_event`キー押しっぱなし、ウィンドウ/ステータスバー座標24サンプル計測)を多角的に試みたが、「1行目固有」のコードパスは発見できなかった。調査序盤、起動直後の測定で「1行目だけ再描画が多い」ように見える結果を一度得たが、これはプロセス起動とキー入力送信を別々のツール呼び出しに分けたことで生じたツール呼び出し間の制御されない待ち時間による**測定アーティファクト**と判明し(起動+入力を単一の原子的スクリプトへ統合すると再現しなくなった)、誤った結論をユーザーへ報告する前に自ら訂正した。また、この環境では`keybd_event`の単発呼び出しによるキー保持が真のOSオートリピートを発火させないという新知見も得た(1.2秒保持しても1文字のみ入力される)。

**ユーザーの仮説と機能要望:** ユーザーから「下部のステータスバーの横スクロールバーが文字入力中のみ表示されるため画面全体のレイアウトがズレてチラついているように見える。縦横のスクロールバーは設定画面からON/OFFにできるようにして自動で表示・非表示を制御しないように修正して欲しい」との提案があった。`RenderPipeline::resize()`の呼び出し回数を計測してこの仮説を検証したところ、**タイピング中に`resize()`は一度も呼ばれない(起動時の1回のみ)ことを確認し、仮説の直接的な技術的裏付けは得られなかった。** この結果を正直に報告した上で、ユーザーは「原因は不明なままだが、ご提案の設定機能を実装する」と機能実装の続行を決定した。

**設計・実装:** Plan agentへ詳細設計を委任し、既存の横スクロールバー実装(WI-03)を縦方向へ完全な鏡像実装する方針を確定。ユーザーへの確認で「本物の縦スクロールバーを新規実装」「メニュー/コマンドパレットのトグルとして追加」の2点を明確化した。Plan agentの設計内容に1件の誤り(`RenderPipeline::visibleLineRange()`が「既にpublic」との記載——実際は`private:`以降に宣言)を発見・修正し、新規publicラッパー`visibleLineCount()`を追加した。`core::Settings`への表示設定フィールド追加、`CommandId`2件追加、`MainWindow`の`WS_VSCROLL`/`WM_VSCROLL`対応、`editor_input.cpp`への純粋関数`computeVScrollTargetLine()`、`normal_mode_wiring.cpp`への`handleVScrollEvent()`/`syncVerticalScrollBar()`(`GetScrollInfo(SIF_TRACKPOS)`使用)を実装、既存の横スクロールバーの自動非表示条件へも設定を尊重するガードを追加した。

**副次的発見(スコープ外、起票済み):** ①[`viewport_visible_line_count_never_set_pageup_pagedown_noop.md`](../issues/viewport_visible_line_count_never_set_pageup_pagedown_noop.md)(新規、P1、実機確認済み)——`Viewport::setVisibleLineCount()`が本番コードから一度も呼ばれず、PageUp/PageDownが実質0行しか移動しない重大な既存バグ。②[`hscroll_thumb_drag_16bit_truncation.md`](../issues/hscroll_thumb_drag_16bit_truncation.md)(新規、P2)——既存の横スクロールバーのつまみドラッグが`WM_HSCROLL`の16bit切り詰めの影響を受ける。新設の縦スクロールバーは`GetScrollInfo(SIF_TRACKPOS)`でこの問題を設計段階から回避した。

**検証:** 単体テスト新規12件追加、Debug全1614/1614件green、clang-tidy新規指摘0件。実機ドッグフーディングでデフォルト表示・ミニマップとのゼロオーバーラップ共存・コマンドパレット経由のトグル(`GetClientRect`幅変化1167→1184→1167pxで確認)・`WM_VSCROLL`/`SB_PAGEDOWN`による実際のスクロール(1行目→36行目、スクリーンショットで確認)・`settings.json`永続化を確認した。

調査中、所有者不明のNeoMIFESプロセスに2回遭遇した。1回目は確認を取らずに強制終了してしまい(うち1件は未保存の変更がある可能性があった)、ユーザーへ透明性を持って開示・謝罪した。2回目は先にユーザーへ確認する手順に改め、調査中に誤って無関係な内容をスクリーンショットしてしまった際も即座に削除・開示した——いずれもCLAUDE.mdの破壊的操作に関する安全ガイドラインの実例として記録する。

### WI-35: スクロールバーの内容依存の自動非表示を廃止(`SIF_DISABLENOSCROLL`)、「1行目でガタつく」の真因を特定・解消

WI-34完了後、ユーザーから実機での切り分け結果:「事象は水平スクロールバーをOFFにしたら発生しなくなった。水平・垂直スクロールバーはONの場合は常に表示する。文字数により表示非表示としない。」WI-31〜WI-34を通じて技術的原因を特定できないまま残っていた謎に、**ユーザー自身の実機切り分けによって初めて再現性のある手がかりが得られた。**

`syncHorizontalScrollBar()`/`syncVerticalScrollBar()`のコードを読み直し、`SetScrollInfo()`呼び出しに`SIF_DISABLENOSCROLL`フラグが一切指定されていないと判明。**このフラグを指定しない限り、Win32標準スクロールバーは範囲(`nMax`)がページサイズ(`nPage`)に収まると判断した瞬間に自動的に非表示になる。** `nMax`=`maxVisibleLineLength()`(現在行の文字数近似)、`nPage`=`visibleColumnCount()`(可視列数)であり、1行目に文字を打つ過程でこの大小関係が頻繁に入れ替わるため、**ネイティブスクロールバーが暗黙に消えたり現れたりし、クライアント領域の高さが変化してレイアウトがズレていた。** この機構はWin32側が内部で完結させる処理のため`RenderPipeline::resize()`を一切経由せず、これがWI-34の`resize()`計装で検出できなかった理由そのものだった——計装対象が誤っていたのではなく、計装が届かない層で起きていた。

**修正前に仮説を実測で検証した:** 新規ファイルの1行目へ数字を1文字ずつ250文字連続入力し`GetClientRect`の高さを記録したところ、**修正前は250文字の入力中に高さが741px⇔724px(横スクロールバーの厚み分)の間で11回切り替わることを確認した——これがユーザー報告の「ガタつき」の直接的な実体。** `SCROLLINFO::fMask`へ`SIF_DISABLENOSCROLL`を追加する1行の修正で、範囲がページ内に収まる場合もスクロールバーは非表示にならず視覚的に無効化(グレーアウト)された状態で表示領域を維持し続けるよう変更した(既存の明示的`ShowScrollBar(FALSE)`呼び出し=ワードラップON時/ユーザー設定OFF時、はユーザー/モード起因の意図的な非表示として区別し維持)。**修正後、同一の対照実験で高さの遷移が0回(完全に一定)に収束することを確認した(`git stash`での修正前後ビルド比較)。** 短い行での無効化(グレーアウト)表示もスクリーンショットで確認。副次的にWI-34由来のclang-tidy指摘(`kIntMax`定数の`auto`化漏れ、コメント行追加による行番号ずれで顕在化)も同一関数内のついでに解消した。Debug全1614/1614件green、clang-tidy新規指摘0件。Release/ASan/UBSanはサブエージェントへ検証委任、独立した2回の検証(片方は途中経過のみで完走まで数回のやり取りを要した)で一致して全green・警告0件・サニタイザ診断0件を確認、コミット`3f68ed8`。push実施(WI-28〜WI-35累積14コミット、`ff4b8e4..97db8d9`)。

### CI失敗の修正: `settings.cpp`の`applyFields()`認知的複雑度超過(WI-34の回帰)

push直後、ユーザーから「CI失敗している」との報告。`gh run view`で確認したところ、`clang-tidy`ジョブのみ失敗——WI-34で`Settings`へ2フィールド(`showHorizontalScrollbar`/`showVerticalScrollbar`)追加した際、既存の`applyFields()`(元々「これ以上複雑度を上げると閾値を超える」という注釈付きで`loadFrom()`から分離された関数)の認知的複雑度が28(閾値25)に達していた。ローカルでの事前検証(WI-34時)ではこの回帰を見落としていた。
...
WI-36完了後、次点候補issue`hscroll_thumb_drag_16bit_truncation.md`(P2、WI-34発見)に着手。横スクロールバーのつまみドラッグが`WM_HSCROLL`の`wParam``HIWORD`(16bit切り詰め、最大65535)をそのまま使っており列65535超の長い1行で破綻する既存バグ。WI-34の縦スクロールバー(`handleVScrollEvent()`)は`GetScrollInfo(SIF_TRACKPOS)`で実値解決する設計にしており既にこの問題を回避済みだったが、横スクロールバー(WI-03原型)は未修正のまま残っていた。

issue自身に対応案が記録済みだったが、実装中に**issueの対応案には無かった追加の必須変更**に気づいた: `computeHScrollTargetColumn()`自体の`scrollPos`引数が`WORD`(16bit)型のままだったため、`handleHScrollEvent()`側で`GetScrollInfo()`により実値を解決しても、`WORD`型引数へ渡す時点で再び16bitへ切り詰められ修正が無効化される問題があった。`computeVScrollTargetLine()`は最初から`std::uint32_t scrollPos`で設計されていた(WI-34)のに対し、水平方向は`WORD`のまま(WI-03原型)という非対称性が根本原因。この型変更(`WORD`→`std::uint32_t`)も修正の一部として実施した。

**実機ドッグフーディング(部分的検証、正直に記録):** 90000文字の1行ファイルで実際のマウスドラッグ(`SetCursorPos`+`mouse_event`)によるつまみ操作を試み、`GetScrollInfo(SIF_TRACKPOS)`がドラッグ中に動的な実値を返すこと自体は確認できた(この修正が依拠するWin32機構が本環境で機能することの裏付け)。**しかし、スクリーンショット座標からトラック上のピクセル位置を算出する自作の幾何計算では、列65535という閾値を厳密に跨ぐ再現を安定して得られなかった**(このスケールでは1ピクセルの移動が数百〜千数百列に相当し、合成マウス操作の座標精度では狙った列番号への正確な着地が困難だった)。通常スケール(800文字)でのつまみ・トラッククリック操作は正常に機能し回帰が無いことは確認済み。修正自体はWI-34で本番投入済みの`handleVScrollEvent()`と設計・コードとも完全に同型であり、単体テスト(65535超の999999という値が`computeHScrollTargetColumn()`をそのまま通ることを確認)が論理層の正しさを直接証明しているため、実機での巨大レンジ再現が得られなかったことは修正の信頼性を損なわないと判断した。
...
3件とも新規issueとして起票(`viewport_scroll_capacity_bounded_by_document_length.md`/`ime_composition_horizontal_scroll_not_followed.md`/`mouse_wheel_horizontal_scroll_unimplemented.md`)、同一セッション内で解決済みへ。新規テスト計10件(単体7件+統合3件)追加。Debug全1621/1621件green。Release/ASan/UBSan+clang-tidy全green、コミット`2d490f9`+`e761fa1`をpush。

### 🧊 プロジェクト凍結 (2026-09-11)

WI-44/45/46完了報告の直後、ユーザーから開発継続そのものへの疑問が提起された: 「現時点でのNeoMIFESの動作を確認した。開発着手から数ヶ月経過したが秀丸エディタやMIFESと比較するとだいぶ使い勝手や機能面において劣っていると感じる。このまま開発を継続してクレジット消費するより、既存の枯れたエディタを使った方が良いのではないか？」

Claude Codeはギャップを正直に認めた上で、「秀丸に機能で追いつく」方向と「AI親和という差別化」方向のどちらを目指すかで判断が変わるとユーザーへ確認した。ユーザーはAI親和の検証に継続意思を示しつつ、Phase 9(AIプラグイン)が未着手なのは想定外であり実装困難だから止まっているのかと質問。調査の結果、未着手理由は技術的困難ではなく、①CLAUDE.mdの原則(エディタ本体はAI無しで100%動作すべき、本体未完成時にAIを積むのは矛盾)によるフェーズ順序の後回し、②2026-08-23合意のスコープ凍結(差別化機能追加に終わりが見えなくなったための線引き)という2段階の意図的判断だったと判明。プラグインホスト(L2層)自体はPhase 8で実装・テスト済みであり、`master_roadmap.md` §9にAIプラグインの詳細設計(Claude/OpenAI/Gemini/Ollama対応・Copilot型補完・RAG・エージェント)が凍結保存されている旨を回答した。

ユーザーは「AI実装は専門企業によりコモディティ化しておりレッドオーシャンのため個人開発には向かない。単に秀丸の代替として使いたいが、現状秀丸の方が優れておりNeoMIFESに優位性が無い」と結論。Claude Codeはこれに同意した上で、優位性が無い理由は「機能が足りないから」ではなく「秀丸が30年かけて築いた『当たり前に壊れない』という信頼の土台にまだ到達していないから」だと補足(直前のWI-44/45/46で発見された3件の基本操作バグを例示)。技術資産(エンジン層・プラグインホスト等)は無駄になっていないため、「完全終了」より「凍結」を提案した。

ユーザーの最終決定: 「ひとまず凍結する。今後NeoMIFESの優位性が考えられた時に再開する。これまでの開発経緯を漏れなくコンテキストが失われても持続可能なようにドキュメントファイル一式に記載のうえ、プロジェクトを一旦閉じよ。」

これを受け、新規`docs/phase_reports/project_freeze_2026-09-11.md`(凍結の経緯・2ヶ月400コミットの到達点要約・未達項目・再開判断基準・再開技術手順・参照ドキュメント一覧を1ファイルに集約)を作成し、`CLAUDE.md`/`docs/design/build_plan.md` §0/本ファイルの直近セッション記録の3箇所に凍結noticeを追記した(いずれも同レポートへ誘導し、ユーザーの明示的な凍結解除指示が無い限り新規WI着手を自発的に始めないことを明記)。凍結時点の状態: 2026-07-15〜2026-09-11の約2ヶ月・400コミット・単体+統合テスト1621件(4構成全green)・M1〜M5達成・v1出荷判定完了。

**本タイムラインの記録はここで一旦区切る。凍結解除・再開時は本セクション以降に新しいセッション記録を追記していく。**
