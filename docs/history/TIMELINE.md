...
**次回 (Phase 5b):** スコープ未確定。候補: Find bar UI配線(新規UI基盤、WC_EDIT子コントロール or 自前描画の設計判断を要する)、インクリメンタル検索(`IncrementalFindService`)、複数行にまたがるマッチ対応、置換(`ReplaceAllCommand`/`ReplaceInFilesCommand`、`application::ICommand`ではなく`core::ICommand`に修正済みのスケッチをdetailed_design.md §7.1''参照)、Grep(複数ファイル横断)、巨大ファイルでのチャンク並列走査・SIMD最適化(今回の実測値を踏まえて要否判断)。**保留中のPhase 4b8**(矩形選択・タブ⇔スペース変換・複数カーソルクリップボードのN対N分配等)に戻る選択肢もあわせて次セッション開始時にユーザーへ提示すること。詳細は `detailed_design.md` §7・`RESUME_HERE.md` §3.16/§6 参照。
