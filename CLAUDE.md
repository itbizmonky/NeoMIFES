...
> # 🧊 プロジェクト凍結中 (2026-09-11) — 作業を始める前に必ずここを読め
>
> **NeoMIFES はユーザーの判断により開発を凍結している。完全終了ではなく、条件が整えば再開する一時停止。** 経緯・理由・再開条件・現在の技術的到達点は [`docs/phase_reports/project_freeze_2026-09-11.md`](docs/phase_reports/project_freeze_2026-09-11.md) に集約した——**まずこのファイルを読むこと。**
>
> **凍結理由の要約:** 当初の差別化構想「AI親和」(Phase 9)は、Copilot/Cursor 等の専門企業によるコモディティ化で個人開発が勝負できる領域ではないとユーザーが判断した。AI 抜きで秀丸エディタ/MIFES と同じ土俵(通常のテキストエディタとしての完成度)で比較すると、現時点で明確な優位性が無い。
>
> **このファイルを読んでいるセッションへ:** ユーザーから明示的に「凍結解除」「再開」の指示が無い限り、新しい実装作業(WI着手・機能追加・issue対応)を自発的に始めないこと。ユーザーから技術的な質問やコード参照を求められた場合の調査対応は通常通り問題ない。凍結解除の意思が示されたら、まず [`docs/phase_reports/project_freeze_2026-09-11.md`](docs/phase_reports/project_freeze_2026-09-11.md) §7 の技術的再開手順に従うこと。
>
> ---
>
...
12. **プロジェクトディレクトリ外(特に `C:\` 直下や `C:\Users\<user>\` 直下)に無断でフォルダ/ファイルを作成しない。** 比較ビルド用の一時チェックアウト・スクラッチ用途は、必ずセッションのスクラッチパッド(`%LOCALAPPDATA%\Temp\claude\...` 配下)か、プロジェクト直下の一時ディレクトリ(作業終了後に削除)を使う。`git worktree` を使う場合も配置先は同様に限定し、作業後は必ず `git worktree remove` で片付ける。ユーザーの環境を汚さないことを最優先する。(2026-08-24、比較検証用に `C:\Users\<user>\` 直下へ無断でworktreeを作成した実例から追記)
