...
OriginalBuffer::openMemoryMapped(const std::filesystem::path& path, std::uint64_t byteOffset) {
    ...
    std::vector<Checkpoint> checkpoints;
    if (!scanUtf8(contentBytes, totalCu, totalNewlines, checkpoints, kCheckpointBytes)) {
...
std::u16string_view OriginalBuffer::viewMemoryMapped(std::uint64_t offset,
    ...
    const std::uint64_t skipBytes = decodeUtf8Run(
        bytes, checkpoint.byteOffset, offset - checkpoint.cuOffset, nullptr, skippedCu);
    ...
    decodeUtf8Run(bytes, decodeStartByte, length, decoded.get(), produced);
