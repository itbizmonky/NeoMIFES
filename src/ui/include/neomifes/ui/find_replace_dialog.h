...
//
// Unlike GrepBar/GotoLineBar/CommandPalette (all WS_CHILD strips docked to
// an edge of the main window, notifying the app layer through
...
// those widgets still use. It owns its OWN window class + WndProc (same
// registration/creation shape as ui::MainWindow), rather than piggybacking
// on MainWindow's message handling the way the remaining embedded bars do.
// ui::FindDialog (WI-24, the Ctrl+F counterpart to this Ctrl+H dialog) later
// adopted the identical top-level-window shape, replacing the original
// embedded-bar ui::FindBar this class was contrasted against when first
// written - see find_dialog.h's own header comment for that migration's
// rationale.
//
...
// being docked at a fixed edge offset the way the remaining embedded bars
// are.
//
// Win32-mechanics-only, same "knows nothing about neomifes::search/
// document/core" separation ui::FindDialog also documents - it deals only in
...
// shape deliberately mirrors FindDialogConfig's onQueryChanged/onFindNext/
// onFindPrevious/onClosed (plus this dialog's own onReplaceCurrent/
// onReplaceAll) so the app layer's buildFindDialogConfig()-style wiring
// logic can be reused almost verbatim for this dialog instead of
// duplicating it.
...
struct FindReplaceDialogConfig {
    // Fired after a 150ms debounce (same FindDialog-established UI-timing
...
public:
    ...
    // (same reasoning as FindDialog's own deleted copy/move).
    ...
    // re-selects" convention as FindDialog::show()).
    ...
    // FindDialog itself already uses (find_navigation.h).
...
private:
    ...
    // reach the stock edit control unchanged. Same role as FindDialog::
    // handleEditKeyDown().
