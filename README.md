...
**開発中。まだ実用に耐えませんが、🎉 M1 (NeoMIFES を NeoMIFES で編集できる) は達成しました。**

| 領域 | 状態 |
|---|---|
| **エンジン層** (Document / Rendering / Search / Encoding / Syntax / Plugin) | ✅ **ほぼ完成** — 起動 148ms / 60fps / 10GB mmap / 100万 Undo / 22 言語ハイライト / RE2 検索 を実測達成 |
| **アプリケーションシェル** (保存 / タブ / メニュー / 設定 / IME) | 🟡 **着手中** — 保存基盤・ファイルライフサイクル UI (Ctrl+S/O/N/D&D)・横スクロールは実装済み・🎉 M1達成。タブ/メニュー/設定/IME は未着手 |
...
**🎉 M1 達成 (2026-08-05):** NeoMIFES で NeoMIFES を編集できるようになりました。実装完了後のドッグフーディングで2件のバグ ([`RESUME_HERE.md` §3.69](docs/handoff/RESUME_HERE.md)) が見つかり修正、その後実際に NeoMIFES で本ファイルを編集・保存・コミットするところまで実証済みです。**WI-03 (横スクロール) も完了しました。次のマイルストーン: WI-04 (`main.cpp` 解体 + 複数文書モデル)。**
...
- 横スクロール (`WM_HSCROLL` ネイティブスクロールバー、長い行の Home/End/入力時自動追従)
- 文字コード自動判定 (UTF-8/16/32, Shift-JIS, EUC-JP, ISO-2022-JP) と行末コード判定
- プラグインエンジン (C ABI DLL、SEH クラッシュ隔離、権限モデル、Job Object サンドボックス)

### 現時点で動かないもの (主なもの)

**タブ (複数ファイル)** / メニューバー / ステータスバー / 行番号表示 / **日本語 IME のインライン変換** / 設定 / テーマ切替 / 全選択 (`Ctrl+A`) / 自動インデント / 自動保存
