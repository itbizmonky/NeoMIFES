...
| **アプリケーションシェル** (保存 / タブ / メニュー / 設定 / IME) | 🟡 **着手中** — 保存基盤・ファイルライフサイクル UI (Ctrl+S/O/N/D&D) は実装済み・🎉 M1達成。タブ/メニュー/設定/IME は未着手 |
...
**🎉 M1 達成 (2026-08-05):** NeoMIFES で NeoMIFES を編集できるようになりました。実装完了後のドッグフーディングで2件のバグ ([`RESUME_HERE.md` §3.69](docs/handoff/RESUME_HERE.md)) が見つかり修正、その後実際に NeoMIFES で本ファイルを編集・保存・コミットするところまで実証済みです。**次のマイルストーン: WI-03 (横スクロール)。**
